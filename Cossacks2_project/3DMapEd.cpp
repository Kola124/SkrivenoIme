...
void PutTexInPoint(int i,SelectionRequest* SR,int T=-1){
	...
	int R;
	if(SR->BrushType){
		R=max(abs(xp-SR->x+yp-SR->y),abs(xp-SR->x-yp+SR->y))-SR->r;
	}else{
		R=Norm(xp-SR->x,yp-SR->y)-SR->r;
	};
	if(R>32)return;
	...
	//conditions checking
	int Hi=THMap[i];
	int WH=min(Hi-TM_Height0,TM_Height1-Hi)*8;
	if(WH<W)W=WH;
	int ANG=GetTerrAngle(xp,yp);
	WH=min(TM_Tan0?ANG-TM_Tan0:100,TM_Tan1-ANG)*8;
	if(WH<W)W=WH;
	//-------------------
	int TX=GetTexture();
	AssignExTex2(i,TexMap[i],TX,W);
	MarkPointToDraw(i);
#else //_USE3D
	if(i<0||i>MaxTH*MaxTH*2)return;
	//definition coordinates and type of point 
	...
	if(TriUnit==16){
		for(int ix=x0;ix<=x1;ix++)
			for(int iy=y0;iy<=y1;iy++){
				...
				int ixx=ix>>ADDSH-1;
				int iyy=iy>>ADDSH-1;
				if(ixx>=0&&ixx<maxmap&&iyy>=0&&iyy<maxmap)minimap[iyy][ixx]=c;
			};
	}else{
		for(int ix=x0;ix<=x1;ix++)
			for(int iy=y0;iy<=y1;iy++){
	...
	for(int i=x0;i<=x1;i+=2)
		for(int j=y0;j<=y1;j+=2){
			...
			if(ix>=0&&ix<maxmap&&iy>=0&&iy<maxmap){
				if(WaterDeep[i+j*MaxWX]>128)minimap[iy][ix]=0xB7;
			};
		};
};
int IMGMAX;
...
void CreateMapPreview(byte* Data,int Lx,int Ly){
	...
	for( i=0;i<MaxSprt;i++){
		if(Sprites[i].Enabled){
			...
			int y=Sprites[i].y;
			SprGroup* SGG=Sprites[i].SG;
			if(SGG==&TREES){//PutMPPoint(x>>5,(y>>5)-(GetHeight(x,y)>>4),0x9B);
			}else/* if(SGG==&STONES)*/PutMPPoint(x>>5,(y>>5)-(GetHeight(x,y)>>4),0x4E);
		};
	};
	int stp1=stp<<1;
	for( i=0;i<msx;i+=stp)
		for(int j=0;j<msy;j+=stp1){
			...
			byte cc=0;
			if(wd>200)cc=0xB4;
			else if(wd>190)cc=0xB5;
			else if(wd>160)cc=0xB6;
			else if(wd>128)cc=0xB7;
			if(cc){
				PutMPPoint(i,j,cc);
				PutMPPoint(i+1,j,cc);
				PutMPPoint(i,j+1,cc);
	...
	for(int i=0;i<msx;i+=2)
		for(int j=0;j<msy;j+=2){
			...
			if(WaterDeep[ofs]>128){
				...
				byte c=0xB3;
				if(B>(6*16-8))c=0xB7;
				else if(B>(5*16-8))c=0xB6;
				else if(B>(3*16-8))c=0xB5;
				else if(B>(1*16-8))c=0xB4;
...
void CreateMapLocking(){
	...
	*/
	CreateLandLocking(120*64/56,true);
	CreateWaterLocking(0,0,(MaxWX)-1,(MaxWX)-1);
#if NMFIELDS>2
//	memcpy(MFIELDS[2].MapV,MFIELDS[0].MapV,MAPSY*BMSX);
//	CreateMFieldLocking(2);
	SmoothFields(MFIELDS+2);
#endif	
	CreateFishMap();
	//Border setting
	int L1=32<<ADDSH;
