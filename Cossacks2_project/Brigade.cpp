...
void Brigade::Rospusk(){
	...
	bool Panic=0;
#ifdef NEWMORALE
	if(WarType&&Morale<32*10000)Panic=1;
#endif //NEWMORALE
	for(int i=0;i<N;i++){
		word mid=Memb[i];
		if(mid!=0xFFFF){
...
int FindUnitsInCell(int cell,int x,int y,int r,int Type,byte Nation,int Need,word* Dest){
	...
	if(cell>0){
		...
		for(int i=0;i<NMon;i++){
			...
			if(MID!=0xFFFF){
				OneObject* OB=Group[MID];
				if(OB&&(!(OB->Guard!=0xFFFF||OB->Sdoxlo||OB->UnlimitedMotion))&&OB->BrigadeID==0xFFFF&&OB->NNUM==Nation&&OB->NIndex==Type&&Norma(OB->RealX-x,OB->RealY-y)<r){
					Dest[N]=OB->Index;
					N++;
					if(N>=Need)return N;
				};
...
bool Brigade::CreateFromGroup(UnitsGroup* UG,int ODIndex){
	OrderDescription* ODS=ElementaryOrders+ODIndex;
	if(UG->N<ODS->NUnits+2)return false;
	OneObject* OB=Group[UG->IDS[0]];
	word Type=Group[UG->IDS[2]]->NIndex;
	word BarbID=Group[UG->IDS[1]]->NIndex;
	bool Dem=false;
	if(BarbID==Type)Dem=true;
	int RX=OB->RealX;
	int RY=OB->RealY;
...
void RotUnitLink(OneObject* OB){
	if(OB->newMons->MotionStyle==3){
		if(OB->NewCurSprite>=OB->NewAnm->NFrames-FrmDec){
			char BestDir=char(OB->LocalOrder->info.MoveFrom.dir);
...
void RotUnit(OneObject* OB,char Dir,byte OrdType){
	if(OB->Transport||OB->newMons->MotionStyle==5||OB->NewBuilding)return;
...
void B_KeepPositionsLink(Brigade* BR){
	...
	if(pri==128){
		...
		for(int i=0;i<N;i++){
			...
			if(MID!=0xFFFF){
				OneObject* OB=Group[MID];
				if(OB&&(!OB->Sdoxlo)&&OB->Attack)return;
			};
		};
	};
	if(BR->PosCreated){
		...
		for(int p=NBPERSONAL;p<BR->NMemb;p++){
			...
			if(MID!=0xFFFF){
				...
				if(OB&&!(OB->InMotion||OB->Sdoxlo)){
					...
					bool FN=0;
					if(!(OB->LocalNewState==State&&(FN=OB->NewAnm->NFrames-FrmDec<=OB->NewCurSprite))){
						Ready=0;
						if(!FN){
							OB->NewCurSprite+=FrmDec<<1;
							if(OB->NewCurSprite>=OB->NewAnm->NFrames)OB->NewCurSprite=OB->NewAnm->NFrames-1;
...
void DrawBorder(Brigade* BR){
	...
	if(BR->WarType&&BR->NMemb>NBPERSONAL&&BR->PosCreated){
		...
		byte cl=clrYello;
		if(BR->BOrder){
			if(BR->BOrder->BLink==&B_KeepPositionsLink&&(!StrelMode||(GetTickCount()-SStartTime)<200)/*REALTIME-BR->LastOrderTime<60*/)Cross=true;
		...
		for(int i=NBPERSONAL;i<N;i++){
			...
			if(MID!=0xFFFF){
				...
				if(OB&&!OB->Sdoxlo){
					...
					yy=OB->RealY>>4;
#ifdef _USE3D
					GPS.ShowGP((OB->RealX>>4)-(mapx<<5)-16,(OB->RealY>>5)-(mapy<<4)-OB->RZ-16,SB_img,0,0);
#else
		...
		if(StrelMode&&(GetTickCount()-SStartTime)>200){
			...
			word LockType=GetBrigLockType(BR);
			int N=PORD.NUnits;
			int* x=PORD.px;
			int* y=PORD.py;
			for(int i=0;i<N;i++){
				...
				if(ofs>=0&&ofs<(1<<(TopSH+TopSH))){
					if(GetTopRef(ofs,LockType)<0xFFFE){
						...
						DrawPlaneLine(xx,yy-2,xx,yy+2,RED_COLOR);
						DrawPlaneLine(xx-2,yy,xx+2,yy,RED_COLOR);
					}
				}
			}
		}
	...
	if(MinFailed){
		...
		for(int i=0;i<N;i++){
			if(Mem[i]!=0xFFFF){
				...
				int xx=x0;
				int yy=y0;
				int v=FindSuperSmartBestPosition(NULL,&xx,&yy,x-xx,y-yy,0,LockType);
				if(v){
					px[i]=xx;
					py[i]=yy;
...
void B_HumanGlobalSendToLink(Brigade* BR){
	...
	if(NextTop==0xFFFF){
		NextTop=GetTopology(xc>>4,yc>>4,lockType);
		if(NextTop==0xFFFF){
			BR->DeleteBOrder();
			return;
		};
	...
	word NextNextTop=GetMotionLinks(FinalTop+NA*NextTop,lockType);
...
void EraseBrigade(Brigade* BR){
	bool PANIC=0;
	#ifdef NEWMORALE
		PANIC=BR->Morale<32*10000&&BR->WarType;
	#endif
		...
		if(NPresence[cell]&mask){
			...
			for(int i=0;i<NMon;i++){
				...
				if(MID!=0xFFFF){
					...
					if(OBJ&&(!(OBJ->NMask&mmask||OBJ->newMons->Artilery||OBJ->UnlimitedMotion))&&Norma(OB->RealX-OBJ->RealX,OB->RealY-OBJ->RealY)<800){
						NEN++;
						if(NEN>=3){
...
#else
					OneObject* OBJ=Group[MID];
					if(OBJ&&(!(OBJ->NMask&mmask||OBJ->newMons->Artilery||OBJ->UnlimitedMotion))&&Norma(OB->RealX-OBJ->RealX,OB->RealY-OBJ->RealY)<512){
						OB->AttackObj(MID,15+128,0);						
						return MID;
					};
#endif
	...
	for(int j=0;j<N;j++){
		...
		if(MID!=0xFFFF){
			...
			if(OB&&OB->Serial==MSN[j]&&!OB->newMons->Artilery){
				if((!(OB->LocalOrder))||(OB->EnemyID==0xFFFF&&!(OB->LocalOrder&&OB->LocalOrder->DoLink==&NewAttackPointLink))){
#else
			if(OB&&OB->Serial==MSN[j]&&!OB->newMons->Artilery){
...
void Brigade::Bitva(){
	...
	Brigade_Bitva* OR1=(Brigade_Bitva*)CreateOrder(1,sizeof Brigade_Bitva);
...
void ProduceByAI(byte nat,word Producer,word ID){
	...
	case ShebekaID:
		...
		break;
	default:
		Group[Producer]->Produce(ID,CITY[nat].DestUnitsGroup);
	};
};
...
word SearchEnemyToDestroy(Brigade* BR){
	...
	if(NS){
		...
		if(CanAttBLD){
			...
			//if(BFND){
				//if(Group[BestID]->Ref.General->MoreCharacter->Shield>50)BestID=0xFFFF;
			//};
			if(BestID==0xFFFF){
				//search for unit
			};
...
void MakeShipBattleLink(Brigade* BR){
	...
	//case 1://go to strong enemy group
		...
		/*
		if(rando()<3000&&FinalTop==-1){
			if(SearchMineToDestroy(BR)==0xFFFF){
				if(OR->Params[0]==-1){
					//need to search final zone
					int NTop=GetNAreas(1);
					int NAtt=0;
		...
		if(FinalTop==-1){
			...
			for(int i=0;i<NWTopNear;i++){
				int top=WTopNear[i];
				int D=GetLinksDist([top*WNAreas+CurTop];
				if(D<DT){
					int F=GetMaxForceOnTheWay(CurTop,top);
					if(MyForce>=(F-(F>>2))){
						DT=D;
						BestTop=top;
					};
