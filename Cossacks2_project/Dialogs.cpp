...
void Canvas::AddBar(int x,int y,int Lx,int Ly,byte c){
	CheckSize(L+100);
	CN_BYTE(0);
	CN_INT(x);
	CN_INT(y);
...
void Canvas::AddLine(int x,int y,int x1,int y1,byte c){
	CheckSize(L+100);
	CN_BYTE(1);
	CN_INT(x);
	CN_INT(y);
...
void Canvas::AddRect(int x,int y,int Lx,int Ly,byte c){
	CheckSize(L+100);
	CN_BYTE(2);
	CN_INT(x);
	CN_INT(y);
...
GPPicture* DialogsSystem::addGPPicture(SimpleDialog* Parent,
...
GP_TextButton* DialogsSystem::addGP_TextButton(SimpleDialog* Parent,int x,int y,char* str,
...
GP_TextButton* DialogsSystem::addGP_TextButtonLimited(SimpleDialog* Parent,int x,int y,char* str,
...
UniversalButton* DialogsSystem::addUniversalButton(int x,int y,int Lx,char* str,int GP_File,
...
GP_Button* DialogsSystem::addGP_Button(SimpleDialog* Parent,int x,int y,int GP_File,int Active,int Passive){
	int i;
	for(i=0;i<MAXDLG&&DSS[i]!=NULL;i++);
	if(i<MAXDLG){
...
void CBar(int x0,int y0,int Lx0,int Ly0,byte c){
	...
	if (!GPS.GetClipArea().ClipRect( x0, y0, Lx0, Ly0 )) return;
	GPS.DrawFillRect( x0, y0, Lx0, Ly0, 
			GPColor::ToARGB( 0xFF, GPal[c].peRed, GPal[c].peGreen, GPal[c].peBlue )  );
#else
...
ppp3:					mov     al,[edi]
			...
			for(int iy=0;iy<BV->Ny;iy++){
				for(int ix=0;ix<BV->Nx;ix++){
					...
					//int addB=(BV->DECLX+BV->OneLx)*BV->Nx-BV->OneLx;
					//int sofst=int(ScreenPtr)+BV->x+BV->OneLx*ix+(BV->y+BV->OneLy*iy)*SCRSizeX;
					CopyTo16(BV->x+BV->OneLx*ix,BV->y+BV->OneLy*iy,(byte*)ps,(BV->OneLx+BV->DECLX)*BV->Nx,BV->OneLx,BV->OneLy);
					/*__asm{
						push	esi
						push	edi
						pushf
...
ppp1:					mov		ecx,Lx
		...
		CopyTo16(BV->x,BV->y,(byte*)ps,BV->OneLx*BV->Nx,Lx<<2,Ly);
		/*int addof=SCRSizeX-(Lx<<2);
		__asm{
			push	esi
			push	edi
...
BPXView* DialogsSystem::addBPXView(SimpleDialog* Parent,int x,int y,int OneLx,int OneLy,int Nx,int Ny,int RealNy,byte* Ptr,VScrollBar* VSC){
	int i;
	for(i=0;i<MAXDLG&&DSS[i]!=NULL;i++);
	if(i<MAXDLG){
...
void ComboBox::AssignScroll(DialogsSystem* DSS,VScrollBar** SCR,int GPFile,int Sprite,int MaxLy){
	*SCR=DSS->addNewGP_VScrollBar(NULL,x1-DSS->BaseX-GPS.GetGPWidth(GPFile,Sprite)+1,y1+5-DSS->BaseY,OneLy*MaxLy+9,1,0,GPFile,Sprite);
	VS=*SCR;
	VS->Visible=0;
	VS->Enabled=0;
	VS->SPos=0;
...
WinComboBox* DialogsSystem::addWinComboBox(SimpleDialog* Parent,char* Message,int x,int y,int Lx,int Ly,
...
int SQPicture::GetLx() const 
{ 
	return GetW(); 
}
int SQPicture::GetLy() const 
{ 
	return GetH(); 
}
...
SQPicture::SQPicture() : GPBitmap() 
{
}

SQPicture::SQPicture(char* Name) : GPBitmap( Name ) 
{
}
#endif // _USE3D
...
void DialogsSystem::ProcessDialogs(){
	...
	for(int i=0;i<MAXDLG;i++){
		...
		if(SD&&SD->Enabled){
			int dd=0;
			try{
				if(SD->ParentSB&&SD->ParentSB->Visible)dd=SD->ParentSB->SPos;
			}catch(...){};
			SD->y-=dd;
			SD->y1-=dd;
			if(SD->Refresh)
...
#else
			if (UseMouse && 
				(mouseX >= SD->x) && (mouseX <= SD->x1) && 
				(mouseY >= SD->y) && (mouseY <= SD->y1) &&
				GPS.GetClipArea().IsPtInside( mouseX, mouseY ))
			{
#endif // _USE3D
		...
		if(SD&&SD->NeedRedrawAll){
			for(int j=0;j<MAXDLG;j++)if(DSS[j])DSS[j]->NeedToDraw=true;
	...
	*/
	...
	if(Hint&&HintFont){
		if(HintX==-1){
			...
			int y0=mouseY+32;
            if (x0 < 10)x0 = 10;
            if (x0 + MaxLx + 20 > RealLx)x0 = RealLx - MaxLx - 20;
            if (y0 + Ly + 16 > RealLy)y0 = RealLy - Ly - 16;
			...
			int x1=HintX;
			CheckFontColor(HintFont);
			int L=GetRLCStrWidth(Hint,HintFont);
			ShowString(x0-(L/2),y0,Hint,HintFont);
		}else{
			int x0=HintX;
			int y0=HintY+UNI_HINTDY1;
			int pos=0;
...
void DialogsSystem::CloseDialogs(){
	for(int i=0;i<MAXDLG;i++){
		...
		if(SD){
			//assert(_CrtCheckMemory());
			if(SD->AllocHint&&SD->Hint)free(Hint);
			if(SD->AllocPtr)free(SD->AllocPtr);
			SD->AllocPtr=NULL;
