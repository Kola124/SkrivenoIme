...
void LoadMapInterface(){
	MapGP=GPS.PreLoadGPImage("Interf2\\ToolBox");
	GPS.PreLoadGPImage("Interf2\\ToolOpt");
	ScrollGP=GPS.PreLoadGPImage("Interf2\\VScroll2");
};
void ClearModes();
...
void CreateTOOL_PARAM_DEFAULT(){
	...
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR00"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
};
//UNIT parameters
int TP_CurNation=0;
...
bool TP_UNIT_CLICK(SimpleDialog* SD){
	...
	byte CC=0x65;
	for(int i=0;i<MAXDLG;i++)if(TOOL_PARAM.DSS[i]&&TOOL_PARAM.DSS[i]->UserParam>=10000){
...
void CreateTOOL_PARAM_UNIT(){
	...
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR01"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
	...
	for(int j=0;j<NUNITS;j++){
		...
		if(UIDS==0xFFFF){
			...
			if(GO->newMons->IconID!=0xFFFF){
				TOOL_PARAM.addGPPicture(NULL,6+xx*44,yy,0,GO->newMons->IconID);
...
void CreateTOOL_PARAM_EDITROAD(){
	...
	TOOL_PARAM.BaseY=y0+31;
	TOOL_PARAM.addBorder(0,-31,InfDX-10,386,48+26,3);
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_ROADEDITOR"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
	...
	for(int i=0;i<N;i++){
		int spt,grp;
		int FID=GetRoadIMG(i,spt,grp);
		if(FID>0&&grp==R_CurGrp){
			ARoadsRef[ActualNRoads]=i;
			GPS.LoadGP(FID);
			GP_Button* GPB=TOOL_PARAM.addGP_Button(NULL,10,Y0-RTEXDY,FID,spt,spt);
			if(ActualNRoads==TP_CurRoad){
				ColoredBar* CB=TOOL_PARAM.addColoredBar(10,Y0-RTEXDY,GPS.GetGPWidth(FID,spt),GPS.GetGPHeight(FID,spt),0xFF);
				CB->Style=1;
				CURY=Y0-RTEXDY;
			};
			GPB->OnUserClick=&ChangeRoad;
			GPB->UserParam=ActualNRoads;
			NN++;
			Y0+=GPS.GetGPHeight(FID,spt);
			ActualNRoads++;
		};
	};
...
void ProcessTOOL_PARAM_EDITROAD(){
	if(TP_REDP){
		...
		if(WheelDelta){
			...
			WheelDelta=0;
			if(TP_CurRoad<0)TP_CurRoad=0;
			if(TP_CurRoad>=N)TP_CurRoad=N-1;
		};
		if(NeedRoadRefresh){
			NeedRoadRefresh=0;
			CreateTOOL_PARAM_EDITROAD();			
		};
		if(RSCRL->SPos!=RTEXDY){
...
bool ON_SPRITE_CLICK(SimpleDialog* SD){
	...
	if((GetKeyState(VK_CONTROL)&0x8000)&&(GetKeyState(VK_SHIFT)&0x8000)){
		for(int i=0;i<MAXDLG;i++)if(TOOL_PARAM.DSS[i]){
...
void CreateTOOL_PARAM_SPRITE(byte ID){
	...
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR02"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
	...
	if(CURSG->NSections){
		...
		TP_CSECT->Center-=5;
		for(int i=0;i<CURSG->NSections;i++)TP_CSECT->AddLine(CURSG->SectNames[i]);
	...
	int MaxLx=InfDX-35;
	TP_STARTSCROLL=NULL;
	int nsect=CURSG->NSections;
	int csect=CurSection[ID];
	cursection=csect;
	int n=nsect?CURSG->NInSect[csect]:CURSG->NSpr;
	Cur_SprID=ID;
	for(int i=0;i<n;i++){
		int sprx=nsect?CURSG->SectContence[csect][i]:i;
		int spr=CURSG->ObjChar[sprx].SpriteIndex;
		if(CURSG->ObjChar[sprx].FixDx){
			int y0=30;
			GPS.SetWhiteFont(CURSG->FileID);
			if(spr>=4096){
				GPPicture* GPP=GPP=TOOL_PARAM.addGPPicture(NULL,xx+CURSG->ObjChar[sprx].FixDx+CURSG->Dx[sprx]
					,y0+CURSG->ObjChar[sprx].FixDy-CURSG->Dy[sprx],CURSG->FileID,spr);
			}else{
				GPPicture* GPP=GPP=TOOL_PARAM.addGPPicture(NULL,xx+CURSG->ObjChar[sprx].FixDx-CURSG->Dx[sprx]
					,y0+CURSG->ObjChar[sprx].FixDy-CURSG->Dy[sprx],CURSG->FileID,spr);
			};
			byte ang=CURSG->ObjChar[sprx].FixDir;
			int r=CURSG->ObjChar[sprx].FixR;
			int xx0=xx+CURSG->ObjChar[sprx].FixDx+((TCos[ang]*r)>>8);
			int yy0=y0+CURSG->ObjChar[sprx].FixDy+((TSin[ang]*r)>>9);
			ColoredBar* CB=TOOL_PARAM.addColoredBar(xx0-4,yy0-4,8,8,PS_COLOR);
			CB->OnUserClick=&ON_SPRITE_CLICK;
			CB->UserParam=sprx+10000;
			CB->Style=1;
		}else{
			int Lx=GPS.GetGPWidth(CURSG->FileID,spr&4095);
			int Ly=GPS.GetGPHeight(CURSG->FileID,spr&4095);
			int dxx,dyy;
			GetRealGPSize(CURSG->FileID,spr&4095,&dxx,&dyy);
			Lx-=dxx;
			Ly-=dyy;		

			if((Nx==0||TotLx+Lx<MaxLx)&&(spr&4095)<CURSG->NSpr){
				Nx++;
				TotLx+=Lx;
				...
				for(int j=i-Nx+1;j<=i;j++){
					...
					int L1=tdx1-tdx0;
					int spr1x=nsect?CURSG->SectContence[csect][j]:j;
					int spr1=CURSG->ObjChar[spr1x].SpriteIndex;
					Lx=GPS.GetGPWidth(CURSG->FileID,spr1&4095);
					int RLX=Lx;
					Ly=GPS.GetGPHeight(CURSG->FileID,spr1&4095);
					int dxx,dyy;
					GetRealGPSize(CURSG->FileID,spr1&4095,&dxx,&dyy);
					Lx-=dxx;
					Ly-=dyy;
					GPPicture* GPP;
					if(spr1>=4096){
						int dx2=CURSG->Dx[spr1x];
						GPP=TOOL_PARAM.addGPPicture(NULL,xx+tdx0+L1/2-dxx+dx2+dx2,yy-dyy+(MaxLy-Ly)/2,CURSG->FileID,spr1);
					}else GPP=TOOL_PARAM.addGPPicture(NULL,xx+tdx0+L1/2-dxx,yy-dyy+(MaxLy-Ly)/2,CURSG->FileID,spr1);
...
void ProcessTOOL_PARAM_SPRITES(){
	...
	if(CURSG->NSections&&cursection!=TP_CSECT->CurLine){
	...
	if(NTreeItems>0){
		int sp=TreeItems[0];
		ObjCharacter* OC=CURSG->ObjChar+sp;
		if(GetKeyState(VK_TAB)&&mouseX<RealLx-256-20){
			if(LMode&&GP_L_IDXS[CURSG->FileID]){
				if(OC->SpriteIndex<4096)GPS.ShowGP(mouseX-(CURSG->Dx[sp]>>2),mouseY-(CURSG->Dy[sp]>>2),GP_L_IDXS[CURSG->FileID],OC->SpriteIndex,0);
				else GPS.ShowGP(mouseX+(CURSG->Dx[sp]>>2),mouseY-(CURSG->Dy[sp]>>2),GP_L_IDXS[CURSG->FileID],OC->SpriteIndex,0);
			}else{
				if(OC->SpriteIndex<4096)GPS.ShowGP(mouseX-CURSG->Dx[sp],mouseY-CURSG->Dy[sp],CURSG->FileID,OC->SpriteIndex,0);
				else GPS.ShowGP(mouseX+CURSG->Dx[sp],mouseY-CURSG->Dy[sp],CURSG->FileID,OC->SpriteIndex,0);
			};
		};
	};
...
void CreateTOOL_PARAM_GSPRITE(){
	...
	int DL=24;
	TOOL_PARAM.addBorder(0,-31,InfDX-10,386,88+DL/*+CURSG->NSections?24:0*/,3);
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_GSPRITE"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
...
void CreateTOOL_PARAM_TEXTURE(){
	...
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR04"),WinTool,WinTool,WinTool,0);	
	//CreateFlags(10,10);
...
void CreateTOOL_PARAM_PIECES(){
	...
	TOOL_PARAM.BaseY=y0+31;
	TOOL_PARAM.addBorder(0,-31,InfDX-10,386,48+26,3);
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR05"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
	TP_Scroll=TOOL_PARAM.addNewGP_VScrollBar(NULL,InfDX-21-16,48+26+1+7,312-9,100,0,ScrollGP,0);
	TP_Scroll->ScrDy=20;
...
void CreateTOOL_PARAM_WATER(){
	...
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR06"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
...
void CreateTOOL_PARAM_BRIGHTNESS(byte type){
	...
	int Y0=36;
	if(type){
		TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR08"),WinTool,WinTool,WinTool,0);
		TOOL_PARAM.addTextButton(NULL,5,Y0,GetTextByID("ED_PARM04"),&YellowFont,&YellowFont,&YellowFont,0);
	}else{
		TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR07"),WinTool,WinTool,WinTool,0);
...
void CreateTOOL_PARAM_RELIEF(byte type){
	...
	int Y0=36;
	if(type){
		TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR09"),WinTool,WinTool,WinTool,0);
		TOOL_PARAM.addTextButton(NULL,5,Y0,GetTextByID("ED_PARM01"),&YellowFont,&YellowFont,&YellowFont,0);
	}else{
		TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_HDR03"),WinTool,WinTool,WinTool,0);
		TOOL_PARAM.addTextButton(NULL,5,Y0,GetTextByID("ED_PARM01"),&YellowFont,&YellowFont,&YellowFont,0);
	};
	CreateFlags(10,10);
	Y0+=24;
...
void CreateTOOL_PARAM_PAINT(){
	...
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_PAINT"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
...
void CreateTOOL_PARAM_GROUND(){
	...
	TOOL_PARAM.addTextButton(NULL,10,-30,GetTextByID("ED_EDITGROUND"),WinTool,WinTool,WinTool,0);
	CreateFlags(10,10);
	...
	if(ReadBMP8(cc,&BM,&GRPTR)){
		GroBV=TOOL_PARAM.addBPXView(NULL,0,Y0+22,225,207,1,1,1,GRPTR,NULL);
		GroBV->DECLX=256-225;
		GroBV->Mode=1;
	};
...
void RenderToResult(int x0,int y0,int x1,int y1){
	if(!ET_TREES_COLOR){
		ET_TREES_COLOR=GetPaletteColor(0,255,0);
		ET_SOFT_COLOR=GetPaletteColor(255,0,0);
		for(int i=0;i<16;i++)ET_MOUNT_COLOR[i]=GetPaletteColor(i*15,i*15,i*15);
	};
	for(int iy=y0;iy<=y1;iy++){
		int ofs=(iy<<TERRSH)+x0;
		for(int ix=x0;ix<=x1;ix++){
			byte b=TerrMap[ofs];
			if(b==255)b=ET_MOUNT_COLOR[TerrHI[ofs]>>4];
			if(SoftArea[ofs])b=trans8[b+(ET_SOFT_COLOR<<8)];
			ResultMap[ofs]=b;
			ofs++;
		};
	};
...
void GenerateMapForMission(char* Relief,char* Ground,char* Mount,char* Soft,int Style);
void ProcessTerrainEditor(){
	// Fonts
