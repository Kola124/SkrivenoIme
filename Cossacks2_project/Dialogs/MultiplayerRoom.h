...
bool MPL_WaitingGame(bool Host,bool SINGLE){
	...
	for(int i=0;i<8;i++){
		...
		MNAME[i]=MENU.addInputBox(NULL,NameX,y1+NameDy+i*26,PINFO[i].name,20,120,24,FWhite,FYellow);
...
#ifdef LIMITOPTIONS
	for(int p=0;p<RMP.NRelief&&p<2;p++){
...
ffe2:;
	...
	for(int i=0;i<8;i++)PINFO[i].Version=dwVersion;
	if(SINGLE){
		NPlayers=1;
		...
		*/
			...
			*/
			if(HostID!=-1){
				...
				if(GPP->CurPage==1){
					...
					PINFO[HostID].CD      =CMGRP1[2]->CurLine;
					PINFO[HostID].StartRes=CMGRP1[3]->CurLine;
					PINFO[HostID].ResOnMap=CMGRP1[4]->CurLine;
					if(VICT->CurLine){
						PINFO[HostID].VictCond=VICT->CurLine+1;
					}else{
					...
					for(int q=NPlayers;q<7;q++)if(PINFO[HostID].COMPINFO[q]){
						word W=PINFO[HostID].COMPINFO[q];
						int Nat=W>>11;
						int Color=W&15;
						int Diff=(W>>8)&7;
					...
					sprintf(PINFO[HostID].MapName,"RN%d %X %X %s %d.m3d",CMGRP1[5]->CurLine,r,s,Nats,op);
					...
					sprintf(CurrentMap,"RN%d %X %X %s %d.m3d",CMGRP1[5]->CurLine,r,s,Nats,op);
					CMGRP1[0]->NLines=NL0;
			...
			for(int i=0;i<NPlayers;i++){
				if(PINFO[i].Host){
					if(PINFO[i].VictCond)
						VICT->CurLine=PINFO[i].VictCond-1;
					else
					...
					StartGTime=PINFO[0].GameTime;
					StartVCond=PINFO[0].VictCond;
		...
		for(int i=0;i<8;i++){
			if(i<NPlayers){
				...
				if(!SINGLE){
					MREADY[i]->SpriteID=8+(PINFO[i].Ready!=0);
					if(i!=NOMREADY){
		...
		if(!Host){
			...
			if(HostID!=-1&&MyID!=-1){
				...
				case 1:
					...
					CMGRP1[2]->CurLine=PINFO[HostID].CD;
					CMGRP1[3]->CurLine=PINFO[HostID].StartRes;
					CMGRP1[4]->CurLine=PINFO[HostID].ResOnMap;
					for(int i=0;i<7;i++){
			...
			if(Host){
				for(int i=0;i<7;i++){
					...
					if(COMPS[i]->Visible&&COMPS[i]->CurLine){
						if(!COMPPREV[i]){
							byte msk=0;
							for(j=0;j<NPlayers;j++)msk|=1<<ColorBack[j]->Nation;
...
//#endif
			...
			if(GPP->CurPage==2){
				...
				/*
				for(int i=0;i<NPlayers;i++)if(PINFO[i].CHKSUMM!=MINS){
					GMREADY=0;
