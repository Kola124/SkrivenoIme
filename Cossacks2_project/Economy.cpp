...
void EconomePostDraw(){
	GPS.ShowGP(PrpX-2,PrpY-40,BordGP,88,0);
	GPS.ShowGP(PrpX+293,PrpY-40,BordGP,89,0);
	int L=GetRLCStrWidth(ECOSELL,&SmallYellowFont);//"SELL"
	ShowString(PrpX+(IconLx+2)-(L>>1),PrpY-35,ECOSELL,&SmallYellowFont);//"SELL"
	L=GetRLCStrWidth(ECOBUY,&SmallYellowFont);//"BUY"
	ShowString(PrpX+(IconLx+2)*8-(L>>1),PrpY-35,ECOBUY,&SmallYellowFont);//"BUY"
};
void ClearEconomy(){
	ECO.InitInterface();
};
