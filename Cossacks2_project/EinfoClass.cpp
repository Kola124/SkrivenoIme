...
void GlobalEnemyInfo::Process(){
	CHECKIT();
	int tt=tmtmt&255;
	for(int i=0;i<MAXPLAY;i++)if(tt==1+(i<<3)&&EINF[i])
		...
		if(WCL&&!((1<<WCL->NI)&Mask)){
			...
			for(int j=0;j<N2;j++){
				if(CL->Visible){
					...
					int y=CL->y;
					int LI=GetLI(x,y);
					int TP_L=GetTopRef(LI-1);
					int TP_R=GetTopRef(LI+1);
					int TP_U=GetTopRef(LI-MaxLIX);
					int TP_D=GetTopRef(LI+MaxLIX);
					OneObject* WOB=NULL;
					...
					if(TP_L<0xFFFE&&TP_R<0xFFFE){
						...
						//else 
						if(TP_L!=TP_R&&GetLinksDist(TP_L+TP_R*NA)>47)NeedDel=true;
					}else
					if(TP_U<0xFFFE&&TP_D<0xFFFE){
						//if(TP_U==0xFFFE||TP_D==0xFFFE)NeedDel=true;
						//else 
						if(TP_U!=TP_D&&GetLinksDist(TP_U+TP_D*NA)>47)NeedDel=true;
					};
					if(NeedDel){
						if(NEnWalls<200){
							WallsX[NEnWalls]=x;
...
void EnemyInfo::CreateBuildSafetyMap(){
	...
	for(int i=0;i<MAXOBJECT;i++){
		OneObject* OB=Group[i];
		if(OB&&(!(OB->Sdoxlo||OB->NMask&msk||OB->Wall||OB->NNUM==7))){
			int x=OB->RealX>>(4+10);
			int y=OB->RealY>>(4+10);
...
void EnemyInfo::ResearchSafeObject(OneObject* OB,int MinR,int MaxR,int pstart){
	...
	int TU=GetTopRef(tofs-TopLx);
	int TD=GetTopRef(tofs+TopLx);
	int TR=GetTopRef(tofs+1);
	int TL=GetTopRef(tofs-1);
	//bool Clear=0;
	//if(!(TU<0xFFFE&&TD<0xFFFE&&GetLinksDist[TU+GetNAreas()*TD]>30&&TU!=TD)){
	//	if(!(TR<0xFFFE&&TL<0xFFFE&&GetLinksDist[TL+GetNAreas()*TR]>30&&TL!=TR))Clear=1;
	//};
//AddSafePt:
	int r0=(MinR>>6)-1;
	if(r0<2)r0=2;
	...
	for(int rr=r0;rr<=r1;rr++){
		...
		for(int p=0;p<N;p++){
			...
			if(xc>0&&yc>0&&xc<TopLx&&yc<TopLx){
				...
				if(GetTopRef(ofst)<0xFFFE&&!((InflMap[ofst]&255)||CheckBar((xc<<2),(yc<<2),4,4))){
...
void EnemyInfo::RegisterHumanShips(){
	...
	ShipsForce=0;
	if(TopAreasDanger){
		memset(TopAreasDanger,0,GetNAreas(1)*2);
	};
...
int EnemyInfo::GetMaxForceOnTheWay(int TopStart,int FinalTop){
	...
	while(TopStart!=FinalTop){
		int NextTop=GetMotionLinks(FinalTop+TopStart*WNAreas,1);
