...
#define FMM_DEFAULTMEMSIZE (2ULL*1024*1024)
...
class FMManager {
	...
	FMMHANDLER* GetFreeHandler(void);		// - вернуть пустой хендл для заполнения
	FMMHANDLER* AddHandler(unsigned size);	// - найти место и вернуть заполненый хендл
