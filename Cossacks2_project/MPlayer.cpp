...
STAGENEXT:
			...
			if(lp[0]==ExDataID&&dwMsgSize<2048){
				...
				int mycl=7;
				for(int j=0;j<8;j++){
					if(PINFO[j].PlayerID==idFrom)mycl=PINFO[j].ColorID;
				};
				assert(j!=7);
				//assert(lp[1]==SUMM);
			...
			*/
				...
				if(lp[0]==PlExitID){
					...
					//sprintf(Prompt,GetTextByID("PLEXIT"),StartMultiplayer.GetNstr(3+ii));
					//PromptTime=400;
				}else
				if(lp[0]=='CHAT'){
				...
				if(lp[0]=='ALLY'){
					if(NATIONS[NatRefTBL[PINFO[ii].ColorID]].NMask&NATIONS[NatRefTBL[MyNation]].NMask){
						memcpy(CHATSTRING,(char*)(lp+2),lp[1]);
						CHATSTRING[lp[1]]=0;
						CHATDPID=idFrom;
					};
				...
				if(lp[0]=='ALIA'&&lp[3]==lp[0]+lp[1]+lp[2]){
					DPID send=lp[1];
					for(int i=0;i<NPlayers;i++)if(PINFO[i].PlayerID==send){
						PlState[i]=lp[2];
...
void NetCash::Add(byte* Data,int size,DPID idTo){
	if(idTo==DPID_ALLPLAYERS){
		for(int i=0;i<NPlayers;i++)if(PINFO[i].PlayerID!=MyDPID)AddOne(Data,size,PINFO[i].PlayerID);
...
bool StartIGame(bool SINGLE){
	...
	if(SINGLE||((!SINGLE)&&SendToAllPlayers((43+8+8+1)*4,BUF,1))){
		//SendToAllPlayers((43+8+8+1)*4,BUF);
		//SendToAllPlayers((43+8+8+1)*4,BUF,1);
		//int ttm=GetRealTime();
		//bool xx;
		//do{
		//	xx=false;
...
int GetPlayerDefaultVictStatus(int idx){
	CreateSumm();
	byte MyC=NatRefTBL[PINFO[idx].ColorID];
	int MaxS=0;
	int NextMaxS=0;;
...
void HandleMultiplayer(){
		...
		do{
			if(DoNewInet){
				...
				if(T1-T0>15000&&!PStart){
					PINGS.InitNetwork();
					PINGS.SetTargetName("gms.2gw.net");
					FPT0=T1;
					PINGS.SendEcho();
					PStart=1;
				};
			...
			ReceiveMessage(lpDPInfo);
			yyy=(GetRealTime()-fff)>>6;
			ttt=(GetRealTime()-zzz)>>6;
			ProcessMessages();
			if((ttt>2&&zzz==fff)||ttt>20){
				RetryNet(ttt>60);
				zzz=GetRealTime();
				RetryAttempts++;
				DWORD ANSW;
				...
				nttm[3]=nttm[1];
				nttm[4]=nttm[2];
				//SendToAllPlayers(20,nttm);
				SendToAllPlayers(20,nttm,0);
				*/
			...
			if(yyy>80){
				...
				if(!MiniActive)DrawStdBar(xc-250,yc,xc+250,yc+30+NPlayers*18+30);
				int y0=yc+12;
				if(!MiniActive)ShowCString(xc,y0-8,PAUSETEXT,&BigWhiteFont);
				y0+=24;
				int Problem=-1;
				...
				for(int i=0;i<NPlayers;i++){
					...
					if(EBufs[i].Enabled){
						if(EBufs[i].RealTime!=NON)strcat(ccc,READYTX);
							...
							if(PlState[i]==-1/*||yyy>930*/){
								if((yyy>850&&GLOBALTIME>40)||yyy>2000){
									//if(IsGameActive()){
									//disconnection detected, i-th player was disconnected
									int NS=0;
									byte ms=0;
									byte PColor=PINFO[i].ColorID;
									//lets calculate how many human enemies still remains
									for(int v=0;v<7;v++)if(NATIONS[v].ThereWasUnit&&NATIONS[v].VictState==0&&v!=PColor&&(!Erased[v])){
									...
									if(NS<2){
										//only one opponent remains and he is disconnected now
										//it is final of the game
										int VS=GetPlayerDefaultVictStatus(i);
										int VSmy=GetPlayerDefaultVictStatus(MyNation);
										if(NPRECV<2){
											//SetGameID("");
										};
										CmdEndGame(PINFO[i].ColorID,VS,0);
										CmdEndGame(MyNation,VSmy,0);
										if(VS==3){
											//SetGameID("");
									...
									char ccx[200];
									sprintf(ccx,GetTextByID("PLEXIT"),PINFO[i].name);
				...
				bool UseF3=0;
				if(Problem==4&&yyy>350)UseF3=1;
				if(Problem==3&&yyy>400)UseF3=1;
				if(Problem==2&&yyy>400)UseF3=1;
				if(UseF3){
					if(!MiniActive)ShowCString(xc,y0,DISCNTX,&WhiteFont);
				};
				yyy=100;
				NMessM=0;
				NSysM=0;
				...
				if(UseF3){
					if(LastKey==VK_F3){
						...
						if(IsGameActive()){
							...
							if(NPRECV>1){
								DontMakeRaiting();
								ShowCentralText("NORATE",300);
							}else{
								StopRaiting();
								ShowCentralText("INCOMTXT",300);
							};
						};
						*/
						IAmLeft();
	...
	/*
	if(yyy>=1000){
		sprintf(Prompt,"Связь разорвана.");
		PromptTime=300;
		NPlayers=1;
		IAmLeft();
		//DontMakeRaiting();
...
bool CheckPingsReady(){
	for(int i=0;i<NPlayers;i++)if(PINFO[i].PlayerID!=MyDPID&&PSUMM.CheckPlayer(PINFO[i].PlayerID)<40)return false;
	return true;
};
bool CheckExistConn(){
	for(int i=0;i<NPlayers;i++)if(PINFO[i].PlayerID!=MyDPID&&PSUMM.CheckPlayer(PINFO[i].PlayerID)==0)return false;
	return true;
};
void PrintBadConn(char* str){
	bool First=1;
	for(int i=0;i<NPlayers;i++)if(PINFO[i].PlayerID!=MyDPID&&PSUMM.CheckPlayer(PINFO[i].PlayerID)==0){
		if(!First)strcat(str,",");
		else strcat(str," ");
		First=0;
		strcat(str,PINFO[i].name);
...
int GetReadyPercent(){
	int MINP=100;
	for(int i=0;i<NPlayers;i++)if(PINFO[i].PlayerID!=MyDPID){
		int P=(PSUMM.CheckPlayer(PINFO[i].PlayerID)*100)/40;
		if(P<MINP)MINP=P;
	};
	return MINP;
};
...
void LoosedPack::DeleteBadPlayers(){
	...
	for(int i=0;i<16;i++){
		...
		if(ID){
			bool Present=0;
			for(int j=0;j<NPlayers;j++)if(PINFO[j].PlayerID==ID)Present=true;
			if(!Present){
