...
//‘ункционалы дл€ определени€ места дл€ здани€
//1-место дл€ стандартного здани€ (ферма, барак,...)
...
void City::RegisterNewUnit(OneObject* OB){
	...
	if(OB){
		...
		if(OB->newMons->Peasant){
			...
			OB->Borg=1;
			Free.AddObject(OB);
			//if(rando()<24000)OB->AutoKill=true;
		}else{
...
void City::EnumUnits(){
	...
	Nat->NArtdep=0;
	for(int i=0;i<4;i++)Nat->NArtUnits[i]=0;
	int xc=CenterX;
	int yc=CenterY;
	...
	for(int i=0;i<MAXOBJECT;i++){
		...
		if(OB&&OB->NNUM==NI&&(OB->Hidden||!OB->Sdoxlo)){
			...
			if(OB->DoWalls&&!OB->Hidden)NBUILDERS++;
			...
			if((NM->Building&&OB->Stage>=ACR->ProduceStages)||(!NM->Building)){
				...
				NewMonster* NM=OB->newMons;
				if(NM->Farm)Nat->NFarms+=OB->Ref.General->MoreCharacter->NInFarm+OB->AddFarms;
				if(!(NM->Building||(OB->Sdoxlo&&!OB->Hidden)))Nat->NGidot++;
			};
			if(OB->Ready||OB->LocalOrder){
				if(NM->ArtDepo)Nat->NArtdep++;
			...
			if(OB->Ready){
				if((!OB->LocalOrder)||(OB->LocalOrder&&OB->LocalOrder->DoLink==&TakeResourceFromSpriteLink)){
					UnBusyAmount[NIND]++;
					int R=Norma((OB->RealX>>11)-xc,(OB->RealY>>11)-yc);
					if(Producer[NIND]!=0xFFFF){
						if(R<Prodradius[NIND]){
							Producer[NIND]=i;
							Prodradius[NIND]=R;
						};
					}else{ 
	...
	int NEEDBLD=12;
	if(NPS>20)NEEDBLD+=(NPS-20)>>3;
	if(NEEDBLD>20)NEEDBLD=20;
	...
	for(int i=0;i<Nat->NMon;i++){
		...
		if(GO->ManualDisable){
			...
			if(GO->LockID!=0xFFFF){
				...
				else Nat->Mon[i]->Enabled=Nat->Mon[i]->CondEnabled;
			};
		};
		if(GO->ManualEnable){
			GO->Enabled=1;
	...
	for(int i=0;i<MaxBrig;i++){
		...
		if(BRIG->Enabled){
				...
				for(int j=0;j<N;j++){
					...
					if(MID!=0xFFFF){
						...
						if(OB&&OB->Serial==BRIG->MembSN[j]){
							if(OB->LocalOrder&&OB->LocalOrder->DoLink!=&AttackObjLink)NAct++;
							NTot++;
						};
					};
				};
				if(NAct<3){
					if(BRIG->BrigDelay>=FrmDec)BRIG->BrigDelay-=FrmDec;
			...
			if(BRIG->WarType){
				...
				if(BRIG->NGrenades<MG){
					if(!BRIG->GrenDelay){
						BRIG->GrenDelay=int(NATIONS[NI].Mon[BRIG->MembID]->newMons->GrenadeRechargeTime)<<3;
					}else{
						BRIG->GrenDelay-=FrmDec;
						if(BRIG->GrenDelay<=0){
							BRIG->NGrenades++;
							BRIG->GrenDelay=int(NATIONS[NI].Mon[BRIG->MembID]->newMons->GrenadeRechargeTime)<<3;
						};
			...
			for(int j=0;j<NMin;j++){
				if(Type[MIDX[j]]==CurRes){
					if(Level[MIDX[j]]<=MaxULevel){
						...
						if(NU>2)NU=2;
						int p=GetMaxCostPercent(Nat->NNUM,Nat->UPGRADE[Nat->UGRP_MINEUP.UIDS[Lev]]->Cost);
						if(Lev==1){
							if(L2PERCENT[NU]>=p){
...
void BuildWallBrain(Idea* ID){
	...
	for(int i=0;i<nw;i+=2){
		int LI=GetLI(wls[i],wls[i+1]);
		WallCell* WCL=WRefs[LI];
		if(WCL&&WCL->Health!=WCL->MaxHealth)goto BuildWallsNext;
	};
	//need to create gates
	if(!MBS->NGateProj){
		MBS->Gates=new short[8];
		short* gat=MBS->Gates;
		int ps=0;
		int tx=MBS->topx;
		int ty=MBS->topy;
		...
		int xg=tx;
		int yg=ty-wrs;
		if(CheckGateUpXY(xg,yg)){
			gat[ps]=xg;
			gat[ps+1]=yg;
			ps+=2;
		}else
		if(CheckGateUpXY(xg-1,yg)){
			gat[ps]=xg-1;
			gat[ps+1]=yg;
			ps+=2;
		}else
		...
		//let us upgrade gates
		int nu=MBS->NGateProj;
		bool done=true;
		for(int nn=0;nn<nu;nn++){
			int xx=MBS->Gates[nn+nn];
			int yy=MBS->Gates[nn+nn+1];
			int LI=GetLI(xx,yy);
			WallCell* WCL=WRefs[LI];
			if(WCL&&WCL->Sprite<32){
				OneObject* OB=Group[WCL->OIndex];
				if(OB&&!OB->LocalOrder)CreateGates(OB);
				done=false;
			};
		};
		BI->Ready=true;
		if(BI->Over){
			if(MBS->M_ID!=0xFFFF&&MBS->MinersID!=0xFFFF){
				OneObject* MIN=Group[MBS->M_ID];
				if(MIN&&MIN->Serial==MBS->M_SN){
					Brigade* MINERS=&ID->CT->Brigs[MBS->MinersID];
					if(MINERS->Enabled&&MINERS->SN==MBS->MinersSN){
						int Over=0;
						int Nin=0;
						int NMM=MINERS->NMemb;
						word* MEMB=MINERS->Memb;
						word* MSN=MINERS->MembSN;
						for(int i=0;i<NMM;i++){
							OneObject* OB=Group[MEMB[i]];
							if(OB&&OB->Serial==MSN[i]){
								if(OB->Hidden)Nin++;
								else if(!(OB->Sdoxlo||OB->UnlimitedMotion)){
									Over++;
									if((!OB->LocalOrder)||(OB->LocalOrder&&OB->LocalOrder->DoLink!=&GoToMineLink)){
										//go to the mine
										OB->GoToMine(MIN->Index,16);
									};
								};
							};
						};
	...
	return;
BuildWallsNext:
	if(MBS->M_ID==0xFFFF)return;
	if(MBS->MinersID==0xFFFF)return;
	OneObject* MIN=Group[MBS->M_ID];
	if(MIN&&MIN->Serial==MBS->M_SN){
		Brigade* MINERS=&ID->CT->Brigs[MBS->MinersID];
		if(MINERS->Enabled&&MINERS->SN==MBS->MinersSN){
			...
			for(int i=0;i<NMM;i++){
				...
				if(OB&&OB->Serial==MSN[i]){
					if(OB->Hidden)Nin++;
						...
						Over++;
						if((!OB->LocalOrder)||(OB->LocalOrder&&OB->LocalOrder->DoLink!=&BuildWallLink)){
							//need to build wall
							int wind=((int(rando())*MBS->NWalls)>>15)<<1;
							OB->BuildWall(MBS->Walls[wind],MBS->Walls[wind+1],16,1,0);
						};
					};
				};
			};
			if(!Over){
				if((!MIN->LocalOrder)||(MIN->LocalOrder&&MIN->LocalOrder->DoLink!=&LeaveMineLink))MIN->LeaveMine(ID->CT->Nat->UID_PEASANT);
			};
			BI->Over=Over;
		};
	};
	...
	if(MMet<128){
		...
		if(EnMID==0xFFFF||tmtmt-OR1->LastEnemyTime>100){
			...
			if(MyTop!=0xFFFF){
				...
				for(int MID=0;MID<MAXOBJECT;MID++){
					//word MID=EnMids[i];
					OneObject* OB=Group[MID];
					if(OB&&(!OB->Sdoxlo)&&(!OB->NMask&mask)&&(OB->newMons->Capture||!OB->Ready)){						int N=GetAmountOfProtectors(OB);
						if(N<5){
							if(N)N=1;
							int HimTop=GetTopology(OB->RealX>>4,OB->RealY>>4);
							if(HimTop!=0xFFFF){
								//can be captured by diversants
								int dst=GetLinksDist(HimTop*NA+MyTop);
		...
		if(EnMID!=0xFFFF){
			...
			if(EOB&&!EOB->Sdoxlo){
				...
				if(HimTop!=0xFFFF){
					int NextTop=GetNextSafeCell(ARM->NI,10,MyTop,HimTop); //GetMotionLinks[HimTop+GetNAreas()*MyTop];
			...
			for(int j=0;j<NArm;j++){
				...
				if(NU>6&&Norma(x-arx,y-ary)<8){
					...
					if(ThemTop!=0xFFFF){
						...
						for(int k=0;k<NL;k++){
							word AIID=LINKS[k+k];
							int ds1=GetLinksDist(AIID+ThemTop*NA);
...
void AI_Army::SendToMostSafePosition(){
	...
	for(int j=0;j<N;j++){
		...
		Area* AR1=GetTopMap(AR->Link[i+i]);
		int x=((AR1->x)<<6)+32;
		int y=((AR1->y)<<6)+32;
...
bool CheckWallForKilling(OneObject* OB){
	if(OB->Wall&&OB->WallX>0&&OB->WallY>0&&OB->WallX<TopLx-1&&OB->WallY<TopLy-1){
		...
		int TPL=GetTopRef(LI-1);
		if(TPL==0xFFFF)TPL=GetTopRef(LI-2);
		int TPR=GetTopRef(LI+1);
		if(TPR==0xFFFF)TPR=GetTopRef(LI+2);
		int TPU=GetTopRef(LI-TopLx);
		int TPD=GetTopRef(LI+TopLx);
		if(TPD==0xFFFF)TPD=GetTopRef(LI+TopLx+TopLx);

		int NA=GetNAreas();
		if((TPL<NA||TPR<NA)&&(TPU<NA||TPD<NA))return true;
		if((TPL==TPR&&TPL<NA)||(TPU==TPD&&TPU<NA))return false;
		if(TPL<NA&&TPR<NA){
			if(GetLinksDist(TPL*NA+TPR)<35)return false;
		};
		if(TPU<NA&&TPD<NA){
			if(GetLinksDist(TPU*NA+TPD)<35)return false;
		};
		return true;
	}else return false;
};
