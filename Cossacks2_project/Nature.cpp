				...
				if(str[0]=='#'){
					...
					if(!strcmp(str,"#END")){
						if(curOT!=-1){
							...
							if(NU){
								...
								for(int p=0;p<ODE->NLines;p++){
									int nn;
									if(nn=ODE->LineNU[p]){
										for(int q=0;q<nn;q++)ODE->Lines[p][q]-=xs;
									};
								};
								...
								if(ODE->SymInv){
									...
									for(int p=0;p<MaxLy;p++)free(Idc[p]);
									if(!Good){
										char c[128];
						...
						if(NEOrders>=128){
							...
							p5=0;
							p6=0;
							int z=sscanf(str,"%s%s%d%d%d%d%d%d",ccc,SY,&p1,&p3,&p2,&p4,&p5,&p6);
							if(z<4){
								char c[128];
	...
	if(F){
		...
		if(z==2){
			if(NCategories>=MaxCategories){
				MaxCategories+=16;
				Categories=(char**)realloc(Categories,NCategories<<2);
			};
