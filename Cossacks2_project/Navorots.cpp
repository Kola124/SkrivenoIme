...
void ShowDestn(){
	...
	for(int i=0;i<NDestn;i++){
		...
		if(x>=0&&y>=0&&x<=Lx&&y<=Ly){
			if(LMode){
				int c=0xD0+MyNation*4;
				int xx=x>>2;
				int yy=y>>2;
				xLine(xx-4,yy-4,xx+4,yy+4,c);
				xLine(xx-4,yy+4,xx+4,yy-4,c);
			}else{
		...
		if(ObjOpt[i]==255){
			...
			if(ID!=0xFFFF){
				...
				if(GreenTime[i]){
					...
					if(OB){
						if(show)DrawColorMarker(OB,ObjColor[i]);
						if(ObjColor[i]==0xD0&&!HealthMode){
							DrawHealth(OB);
						};
					};
			...
			if(x&&y){
				if(ObjOpt[i]<GPS.GPNFrames(AnmCursorGP)){
					...
					if(!LMode)GPS.ShowGP(x-(mapx<<5)-24,(y>>1)-(mapy<<4)-17-dy,AnmCursorGP,spr,0);
					ObjOpt[i]++;
				}else{
					GreenObjs[i]=0xFFFF;
					GreenTime[i]=0;
