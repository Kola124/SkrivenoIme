...
void CreateInfoMap(){
	...
	NMines=0;
	MineList=znew(int,256);
	MaxMine=256;
	for(int i=0;i<MaxSprt;i++){
		OneSprite* OS=&Sprites[i];
		if(OS->Enabled&&OS->SGIndex!=0xFFFF){
			SprGroup* SG=OS->SG;
			ObjCharacter* OC=&SG->ObjChar[OS->SGIndex];
			if(OC->IntResType<8){
				if(NMines>=MaxMine){
...
bool CheckStoneSklad(int x,int y){
	...
	for(int i=0;i<NSKLAD;i++){
		if(Norma(int(SKLADSX[i])-x,int(SKLADSY[i])-y)<8)return false;
	};
	int ofst=x+y*VAL_SPRNX;
...
bool CheckFarm(int x,int y){
	if(x<=1||y<=1||x>=SsMaxX-1||y>=SsMaxY-1)return false;
	int ofst=x+y*VAL_SPRNX;
	if(CantBuild[ofst]&CB_Farm)return false;
	return 1;//InfoMap[ofst]&4;
};
bool CheckBuilding(int x,int y){
	if(x<=1||y<=1||x>=SsMaxX-1||y>=SsMaxY-1)return false;
	int ofst=x+y*VAL_SPRNX;
	if(CantBuild[ofst]&CB_Building)return false;
	return 1;//(InfoMap[ofst]&4)&&(InfoMap[ofst+1]&4)&&(InfoMap[ofst+VAL_SPRNX]&4)&&(InfoMap[ofst+VAL_SPRNX+1]&4);
};
byte NPORTS;
short PORTSX[32];
short PORTSY[32];
		...
		for(int j=0;j<N;j++){
			...
			if(xx>0&&yy>0&&xx<SsMaxX&&yy<SsMaxY){
				if(!GNFO.EINF[CURRENTAINATION]->GetSafeVal(xx,yy)){
					int top=GetTopRef(xx+xx+(yy<<(TopSH+1)));
					if(top<0xFFFE){
						int TDST=GetLinksDist(MyTop+top);
						if(top==REALMYTOP)TDST=0;
						if(TDST-PrevTopDst<60){
