	...
	if(roomType==TitleRoom){
		if(strstr(message,"@@@[ADM@XYL]")){
			...
			if(message[0]=='@'&&message[1]=='@'&&message[2]=='@'){
				const char* cc=strstr(message,"@@@NFO \\$flags$\\");
				if(cc){
					cc+=16;
					if(strchr(cc,'g')){
				...
				if(cc){
					...
					do{
						...
						for(int i=0;i<TPE->ABPL.NPlayers;i++)if(!strcmp(TPE->ABPL.Names[i],cc3))PRESENT=1;
...
void PEER_ConnectCallback(
	...
	if(success){
		...
		peerSetTitle(TPE->Peer,"cossacks","p2vPkJ", "cossacks", "p2vPkJ", 20, PINGS, PINGS_CROSS);
...
void ChatQueue::Add(char* nick,char* Message){
	...
	CStr[NCStr].Message=znew(char,strlen(Message)+1);
	strcpy(CStr[NCStr].Message,Message);
	CStr[NCStr].PlName=znew(char,strlen(nick)+1);
...
void TotalPeerEngine::AddRoomInGame(char* s){
	...
	if(cc1){
		...
		NRInGame++;
		RIG->Name=znew(char,strlen(ccc1)+1);
...
void AbsentPlayers::AddPlayer(char* Nick){
	...
	if(InsIndex==-1){
		Names[NPlayers]=znew(char,strlen(Nick)+1);
		...
		Names[InsIndex]=znew(char,strlen(Nick)+1);
...
bool GetPLIP(char* Nick,char* IP){
	IP[0]=0;
	peerGetPlayerIP(TPEN.Peer,Nick,&PEER_GetPlayerIPCallback,IP,PEERTrue);
	return IP[0]!=0;
};