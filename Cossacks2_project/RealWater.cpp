...
void GenerateSurface(int x,int y,int Lx,int Ly){
	for(int i=0;i<256;i++){
		SetPoint(Wave0,x+rand()%Lx,y+rand()%Ly,5+(rand()&3),((rand()&511)-256)*110/2);
	};
	memcpy(Wave1,Wave0,sizeof Wave0);
	memcpy(Wave2,Wave0,sizeof Wave0);
	return;
	int pos,yy;
...
void SetPoint(short* Wave,int x,int y,int r,int h){
	...
	int r1=r0*2;
	double rrr=r0*r0/6;
	if(x<=r1||y<=r1||x>=WaveLx-r1||y>=WaveLy-r1)return;
	int pos=x+y*WaveLx;
	for(int i=-r1;i<=r1;i++)
		for(int j=-r1;j<=r1;j++){
...
#define ash 6
void PreProcessWater();
void InitWater(){
	for(int i=0;i<2048;i++){
		if(i<1024+ash-12)Colors4[i]=0xB0B0B0B0;
		else
		if(i>=1024+ash+10){
			int dcc=(i-1024-ash-10);
			if(dcc<2)Colors4[i]=0xBBBBBBBB;
			else Colors4[i]=0xBABABABA;
		}else{
			int c1=0xB0+((i-1024-ash+12)>>1);
			if(i&1)
			Colors4[i]=c1+((c1+1)<<8)+(c1<<16)+((c1+1)<<24);
			else Colors4[i]=c1+((c1)<<8)+(c1<<16)+((c1)<<24);
		};
	};
	...
	for(int i=0;i<10;i++){
		int xx=rand()&255;
		int yy=rand()&255;
		SetPoint(Wave0,xx,yy,2,500);
		SetPoint(Wave1,xx,yy,3,4000);
		SetPoint(Wave2,xx,yy,2,7500);
		xx=rand()&255;
		yy=rand()&255;
		SetPoint(Wave0,xx,yy,2,-500);
		SetPoint(Wave1,xx,yy,3,-4000);
	...
	*/
	/*
	ResFile f1=RReset("Wave0.dat");
	if(f1==INVALID_HANDLE_VALUE){
		ErrM(Could not load Wave0.dat");
	};
	RBlockRead(f1,ScWave0,65536*2);
	RClose(f1);
	f1=RReset("Wave1.dat");
	if(f1==INVALID_HANDLE_VALUE){
		ErrM(Could not load Wave1.dat");
	};
	RBlockRead(f1,ScWave1,65536*2);
	RClose(f1);
	f1=RReset("Wave2.dat");
	if(f1==INVALID_HANDLE_VALUE){
		ErrM(Could not load Wave2.dat");
	};
	RBlockRead(f1,ScWave2,65536*2);
	RClose(f1);
	*/
...
/*
void ShowWater(int x,int y,int Wx,int Wy,int Nx,int Ny,short* Wave){
	int ofst=int(ScreenPtr)+y*ScrWidth;
	int wofst=(Wx+Wy*WaveLx)<<1;
	int Wadd=WaveLx-Nx;
	int Sadd=(ScrWidth<<1)-(Nx<<2);
	int cnt1;
	__asm{
		push	esi
		push	edi
...
lpp2:	xor		eax,eax
		xor		ebx,ebx
		mov		al,[DeepWater+edx*4]
		cmp		al,250
		//jae		NextTetrada
		xor		ecx,ecx
		mov		cx,[esi+edx*2+WaveLx*4]
		mov		bx,[esi+edx*2-WaveLx*4]
		sub		cx,bx
		test	cx,0x8000
		jz		uuu1
		or		ecx,0xFFFF0000
...
void InitCost(){
	...
	for(int i=0;i<32*32;i++){
		...
		//if(rand()<16384)CostPos[i]|=32768;
	};
	
	for(int x=0;x<32;x++)
		for(int y=0;y<32;y++){
			CostPos[GOF(x,y)]=(int(PreCost[GOF(x+1,y)])+int(PreCost[GOF(x-1,y)])+int(PreCost[GOF(x,y-1)])+int(PreCost[GOF(x,y+1)]))>>2;
			//if(rand()<16384)CostPos[GOF(x,y)]|=32768;
		};
	for(int p=0;p<10;p++){
		int xx=rand()&31;
		int yy=rand()&31;
...
int GetFXWave(double x,double y){
	return 0;
	x*=3.1415/32.0;
	y*=3.1415/32.0;
	double t=double(GetTickCount())/1000.0;
	return int((ExSin(x*5+y*3-t)+ExSin(x*7-y*5+t*2)+ExSin(x*6-y*8-t))*356.0);
	//return int((ExSin(x*5+y*3-t)+ExSin(x*5+y*3+t)+ExSin(x*3-y*5-t)+ExSin(x*3-y*5+t))*200.0);
	//return int((ExSin(x*6-t)+ExSin(y*6-t))*300.0);
};
void ProcessCost(){
	for(int i=0;i<32*32;i++){
	//CostHi[i]=GetFXWave(i&31,i>>5);
		word CP=CostPos[i];
		if(CP&32768){
...
void GenerateHi(short* Wave,int x,int y,int h){
	if(x>4&&y>4&&x<WaveLx-5&&y<WaveLy-5){
		...
		Wave[ofst-WaveLx]+=h;
/*		h=(h+h+h)>>2;
		Wave[ofst+1+WaveLx]+=h;
		Wave[ofst+1-WaveLx]+=h;
		Wave[ofst-1+WaveLx]+=h;
		Wave[ofst-1-WaveLx]+=h;
		h=(h+h+h)>>2;*/
/*		Wave[ofst+2-WaveLx*2]+=h;
		Wave[ofst+2]+=h;
		Wave[ofst+2+WaveLx*2]+=h;
		Wave[ofst-2-WaveLx*2]+=h;
		Wave[ofst-2]+=h;
...
void DrawAllWater(){
	...
	for(int i=0;i<smaply;i++){
		for(int j=0;j<smaplx;j++){
			...
			int xx=j+mapx;
			int yy=i+mapy;
			if(!(yy&1))DynDrawWaterFlow(j<<5,(i<<4)+smapy,xx,yy>>1);
			cpos++;
#else
			int xx=j+mapx;
			int yy=i+mapy;
...
bool INSI(int x,int y){
	if(x>=0&&y>=0&&x<smaplx&&y<smaply)return true;
	else return false;
};
void HandleShift(){
	if(mapx==OldMapx&&mapy==OldMapy)return;
	int x0,y0,x1,y1,x2,y2,Lx0,Ly0,Lx1,Ly1,Lx2,Ly2,xs,ys;
	int Dx,Dy;
	bool Is0,Is1,Is2;
	...
	Dy=mapx-OldMapx;
	if(Is2){
		//assert(INSI(x2,y2)&&INSI(x2+Lx2-1,y2)&&INSI(x2,y2+Ly2-1)&&INSI(x2+Lx2-1,y2+Ly2-1)&&Lx2&&Ly2);
		x2++;
		y2++;
		xs++;
		ys++;
	...
	if(Is1&&Lx1&&Ly1){
		...
		//GenerateSurface(x1<<3,y1<<3,Lx1<<3,Ly1<<3);
	};
	if(Is0&&Lx0&&Ly0){
		ExtrapolateSurface(x0,y0,Lx0,Ly0);
		//GenerateSurface(x0<<3,y0<<3,Lx0<<3,Ly0<<3);
	};
	OldMapx=mapx;
	OldMapy=mapy;
};
...
void ProcessWaves(short* Wave0,short* Wave1,short* Wave2){
	...
	//memset(Wave2,0,WaveLx*WaveLy*2);
//	if(tttx)FastProcess1_1(Wave0,Wave1,Wave2);
//	else FastProcess1_0(Wave0,Wave1,Wave2);
...
void HandleWater(){
	...
	//	int xx=rand()&511;
	//	int yy=rand()&511;
	//	GenerateWave(xx,yy,0,0,rand()&63);
	//};
	int WLX=smaplx+2;
	int WLY=smaply+2;
	int han=tmtmt&1;
	...
	ProcessCost();
	/*
	ProcessWaveFrame(Wave0,1,1,WaveLx-2,WaveLy-2);
	ProcessWaveFrame(Wave1,1,1,WaveLx-2,WaveLy-2);
	ProcessWaveFrame(Wave2,1,1,WaveLx-2,WaveLy-2);
	
	for(int px=1;px<16;px++){
		for(int py=1;py<16;py++){
			int ofst=((px*WaveLx)>>4)+(rand()&31)-15+(((py*WaveLy)>>4)+(rand()&31-15))*WaveLx;
			Wave0[ofst]=WaterLevel;
			Wave1[ofst]=WaterLevel;
			Wave2[ofst]=WaterLevel;

		};
	};
	*/
	//DrawCost(20,20,Wave0+20+20*WaveLx,150,150,120,120);
	DrawAllWater();
	/*
	memset(Wave0,0,WaveLx*2);
	memset(Wave1,0,WaveLx*2);
	memset(Wave2,0,WaveLx*2);
	memset(Wave0+WaveLx*(WaveLx-2),0,WaveLx*2);
	memset(Wave1+WaveLx*(WaveLx-2),0,WaveLx*2);
	memset(Wave2+WaveLx*(WaveLx-2),0,WaveLx*2);
	*/
	CorrectLeftWaves();
};
//EDITOR FUNCTIONS
int LastWaterchange=0;
void SetGroundTexRound(int TxIdx,int x0,int y0,int r,int dr);
void SetWaterSpot(int x,int y,int r){
	int dr=CostThickness<<4;
	if(!r)r=1;
	SetGoodDeepSpot(x,y,r<<5,dr,250);
#ifndef _USE3D
	SetGroundTexRound(-1,x,y,(r<<5)+64,64);
#endif
	LastWaterchange=GetTickCount();
};
void EraseWaterSpot(int x,int y,int r){
	int dr=CostThickness<<4;
	ClearGoodDeepSpot(x,y,r<<5,dr,250);
#ifndef _USE3D
	SetGroundTexRound(-1,x,y,(r<<5)+64,64);
#endif
	LastWaterchange=GetTickCount();
...
extern byte* FlowTex;
//byte w_clr[8]={0xB1,0xB2,0xB3,0xB4,0xB5,0xB5,0xB7,0xB8};
byte w_clr[8]={11,248,112,124,87,130,17,125};
//byte w_clr[8]={0xB1,0xB2,0xB3,0xB4,0xB5,0xB6,0xB7,0xB8};
//byte w_clr[8]={75,56,56,77,87,157,184,28};
void CreateWTextureOLD(){
	short* wave;
	switch(CurStage){
	case 0:
		wave=Wave0+WaveLx+2;
...
TTT2:	mov  al,[w_clr+eax]
	...
	for(int i=0;i<128;i++){
		memcpy(FlowTex+128+(i<<8),FlowTex+(i<<8),128);
		memcpy(FlowTex+(i<<8)+32768,FlowTex+(i<<8),128);
		memcpy(FlowTex+128+(i<<8)+32768,FlowTex+(i<<8),128);
	};
};
void CreateWTexture(){
	short* wave;
