...
void ShowBlob(int x,int y,byte* Blob,int Lx,int Ly){
...
#else
	if (!GPS.GetClipArea().IsPtInside( x, y )) return; 
	byte*	bof = Blob;
	int		BLX = Lx;
	int		BLY = Ly;
	...
	if (x + Lx > vp.x + vp.width)
	{
		BLX -= x + Lx - vp.x - vp.width;
	};
	if ( y + Ly > vp.y + vp.height)
	{
		BLY -= y + Ly - vp.y - vp.height;
	};
	if (BLX <= 0 || BLY <= 0)
	{
		return;
...
void Blob::Show(int x,int y,int m){
	...
	//Fast_DrawGrayMask(x-(Lx>>1),y-(Ly>>1),Data+Lx*Ly*m,Lx>>2,Ly,Lx);
	//Fast_DrawGrayIndexMask(x,y,m);
};
//------------------------LOADING&TEST-----------------
Blob Blob1;
void LoadBlobs(){
...
lpp3:		mov		CurBlob,eax
	...
	if(CurBlob>=MaxBlob)CurBlob=0;
	if(Cur==-1)return;
	BlobX[Cur]=x;
	BlobY[Cur]=y;
	BlobVx[Cur]=BDVX[Dir];
	BlobVy[Cur]=BDVY[Dir];
	if(dir2)BlobTime[Cur]=(Blob1.N)<<1;
	else BlobTime[Cur]=(Blob1.N);
	BlobOpt[Cur]=dir2;
};
