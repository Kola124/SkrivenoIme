...
bool ParkWaterNewMonster(OneObject* OB,int x,int y,byte Prio,byte OrdType){
	...
	if(NM->Building)return false;
	if(OB->LockType!=1)return false;
	MotionField* MFI=MFIELDS+OB->LockType;
	int LX=OB->Lx;
	int xx=(x>>8)-(LX>>1);
	int yy=(y>>8)-(LX>>1);
...
void LeaveShipLink(OneObject* OB){
	...
	if(OB->NInside){
		...
		if(!CheckBar(CX+DX-1,CY+DY-1,3,3)){
			...
			INs->NewMonsterSendTo((CX+DX)<<8,(CY+DY)<<8,0,2+128);
			INs->ClearOrderedUnlimitedMotion(2);
			INs->NewMonsterSendTo((CX+DX+(DX>>2)+(rando()&3)-1)<<8,(CY+DY+(DY>>2)+(rando()&3)-1)<<8,0,2+128);
	...
	if(TransportID!=0xFFFF){
		...
		if(TRA&&TRA->Serial==TransportSN){
			if(MovingToCost){
				...
				if((!OR1)&&CheckAbsorbAbility()){//parking is performed
					if(!MidPresent)FindConcPoint();
					MovingToCost=false;
					AbsorbUnits=true;
				};
			};
			if(AbsorbUnits&&MidPresent){
				...
				if(!TRA->delay){
					...
					for(int i=0;i<NUnits;i++){
						...
						if(MID!=0xFFFF){
							OneObject* OB=Group[MID];
							if(!OB->Hidden)if(OnTheWay[i])OnWay++;
						};
					};
			...
			if(ConcPresent){
				...
				//checking units that are on the way,but transport is full
				if(TRA->NInside==ADC->MaxInside){
			...
			if(MovingToFinalCost){
				Order1* OR1=TRA->LocalOrder;
				if((!OR1)&&CheckUnloadAbility()){//parking is performed
					MovingToFinalCost=false;
					ExtractUnits=true;
					ReadyToDrive=false;
				};
...
bool CreateTransProcess(OneObject* TRA){
	...
	bool IsAdded=false;
	for(int i=0;i<MaxTP;i++){
		if(TRAPR[i].Used&&TRA->Index==TRAPR[i].TransportID){
			//process already exists, we need to update units
			TransProcess* TRP=TRAPR+i;
			int N=NSL[TRA->NNUM];
			word* SMon=Selm[TRA->NNUM];
			for(int p=0;p<N;p++){
				word MID=SMon[p];
				if(MID!=0xFFFF){
					OneObject* OB=Group[MID];
					if(OB){
						for(int k=0;k<MaxTP;k++)if(k!=i){
							TransProcess* TRK=TRAPR+k;
							TRK->ExcludeUnit(OB->Index);
						};
						IsAdded=true;
						NewMonster* NM=OB->newMons;
						if((!NM->Building)&&(!OB->LockType)){
							TRP->AddUnit(OB->Index);
						};
					};
				};
			};
...
void CreateUnloadProcess(OneObject* OB,int x,int y){
	...
	for(int i=0;i<MaxTP;i++){
		if(!TRAPR[i].Used){
			TransProcess* TRP=TRAPR+i;
			TRP->Init(OB->Index);
			TRP->SetFinalPoint(x,y);
			TRP->MovingToFinalCost=true;
		};
	};
};
void HandleTransport(){
	for(int i=0;i<MaxTP;i++){
		if(TRAPR[i].Used)TRAPR[i].Process();
	};
};
GOrder::GOrder(){
...
bool OneObject::GoToTransport(word ID,byte Prio){
	if(InArmy&&BrigadeID!=0xFFFF){
		...
		if(BR->Enabled){
			...
			if(OB3)OB3->GoToTransport(ID,Prio);
		};
	};
...
void GoToTransportLink(OneObject* OBJ){
	...
	if((OB&&(OB->DstX<=0||OB->Sdoxlo))||(!OB)){
		...
		if(OBJ->UnlimitedMotion){
			OBJ->Die();
			//OBJ->Sdoxlo=300;
			//OBJ->NewAnemation=&OBJ->newMons->DeathLie3;
		};
		return;
	};
    int xx, yy;// xm, ym;
	...
	if(OB&&OB->Serial==OSN&&OB->NInside<ADC->MaxInside+OB->AddInside){
		...
		if(Norma(OBJ->RealX-OB->RealX,OBJ->RealY-OB->RealY)<OBJ->newMons->StopDistance+512&&OBJ->UnlimitedMotion){
			//hiding;
		...
		if(dst<OBJ->newMons->StopDistance){

			OBJ->UnlimitedMotion=false;
			OBJ->NewMonsterSendTo(OB->RealX,OB->RealY,16,1+128);
			/*
		...
		if(OBJ->UnlimitedMotion){
			...
			OBJ->ClearOrders();
			OBJ->UnlimitedMotion=false;
			OBJ->NewMonsterSendTo((XX+OB->RealX)>>1,(YY+OB->RealY)>>1,16,0+128);
			OBJ->ClearOrderedUnlimitedMotion(2);
			OBJ->NewMonsterSendTo(XX+(rando()>>6),YY+(rando()>>6),16,2+128);
			//OBJ->ClearOrderedUnlimitedMotion(2);
			OBJ->UnlimitedMotion=true;
		}else{
			OBJ->DeleteLastOrder();
...
void OneObject::LeaveTransport(word Type){
    ...
    OR1->info.BuildObj.ObjX=6>>SpeedSh;
...
void LeaveTransportLink(OneObject* OBJ){
	...
	OB->NewMonsterSendTo((OBJ->DstX*3+OBJ->RealX)>>2,(OBJ->DstY*3+OBJ->RealY)>>2,16,2+128);
	OB->ClearOrderedUnlimitedMotion(2);
	OB->NewMonsterSendTo(OBJ->DstX+(rando()&511)-256,OBJ->DstY+(rando()&511)-256,16,2+128);
...
void OneObject::Fishing(){
	if(LocalOrder||StopBuildMode)return;
	PrioryLevel=0;
	Order1* OR1=CreateOrder(0);
	if(OR1){
