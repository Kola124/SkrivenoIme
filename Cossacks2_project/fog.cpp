...
void LoadFog(int set){
	...
	NatPal[32]=GetPaletteColor(255,255,0);
	NatPal[33]=GetPaletteColor(180,180,0);
	NatPal[34]=GetPaletteColor(100,100,0);
	NatPal[35]=GetPaletteColor(35,35,0);

	NatPal[36]=GetPaletteColor(0,255,0);
	NatPal[37]=GetPaletteColor(0,180,0);
	NatPal[38]=GetPaletteColor(0,100,0);
	NatPal[39]=GetPaletteColor(0,35,0);
};
void SetupFog(){
	for(int i=0;i<1024;i++)FOGOFFS[i]=i*FMSX;
};
...
void ShowSuperFluentFog32_160_16(int x,int y,int z1x,int z2x,int z3x,int z4x){
	if(MiniMode)return;
	if(x>=RealLx||y>=RealLy||x<-31||y<-15)return;
	int z1=z1x<<16;
	int	z2=z2x<<16;
	int z3=z3x<<16;
	int	z4=z4x<<16;
...
int GetInterpFOW(int x,int y){//x,y - pixel coordinates
	...
	int fy=y>>6;
	int ms=FMSX-FMSHI-1;
	if(fx<0||fy<0||fx>=ms||fy>=ms)return 0;
	int dx=x&127;
	int dy=y&63;
	...
	return F1+(((F2-F1)*dx)>>7)+(((F3-F1)*dy)>>6)+(((F4+F1-F3-F2)*dx*dy)>>13);
};
void CreateFogImage(){
	//return;
	maxWFX=smaplx;
...
void DrawLModeFog(){
	...
	int ofx=mapx>>2;
	int ofy=mapy>>2;
	int OF0=ofx+3+(ofy+3)*FMSX;
	int sx0=((ofx<<2)-mapx)<<3;
	int sy0=((ofy<<2)-mapy)<<2;
	for(int i=0;i<ny;i++){
		for(int ix=0;ix<nx;ix++){
			ShowSuperFluentFog32_160_16(sx0+(ix<<5),sy0+(i<<4),GetF(fmap[OF0+ix]),GetF(fmap[OF0+ix+1]),GetF(fmap[OF0+ix+FMSX]),GetF(fmap[OF0+ix+FMSX+1]));
		};
		OF0+=FMSX;
	};
};
...
void Init3DFog(){
	...
	F3D_Lx=Nx;
	F3D_Ly=Ny;
	FOGMESH->create((Nx+1)*(Ny+1),6*Nx*Ny,vfTnL);
	word ID=IRS->GetShaderID("fow");
