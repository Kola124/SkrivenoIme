...
////////////////
CEXPORT
UnitsGroup* GetUnitsGroup(GAMEOBJ* Group){
	if(!Group||Group->Type!='UNIT'||Group->Index>=SCENINF.NUGRP) return NULL;
...
CEXPORT bool DetSheep(word NIndex){
	if(NIndex<NATIONS->NMon){
		return NATIONS->Mon[NIndex]->newMons->LockType;
	}
	return false;
};
...
long int GetGroupPower(GAMEOBJ* Grp){
	if(Grp->Type!='UNIT'||Grp->Index<0||Grp->Index>=SCENINF.NUGRP) return -1;
	...
	for(int i=0;i<N;i++){	
		...
		if(OB&&OB->Serial==SNS[i]&&!OB->Sdoxlo){
			life+=OB->Life;
			dam+=OB->Ref.General->MoreCharacter->MaxDamage[0];
		}
	}
	return life*dam;
}
		...
		if(OB&&!(OB->Sdoxlo||OB->NewBuilding)){							
			...
			if(z<MaxSize){
				...
				if(!(OB->NMask&MASK)){
					...
					if(top<0xFFFE){
						Life[top]+=OB->Life;
						AdvCharacter* Adv=OB->Ref.General->MoreCharacter;
			...
			if(z<MaxSize){
				...
				if(!(OB->NMask&MASK)){
					...
					if(top<0xFFFE){
						...
						// strelok
						if(OB->Ref.General->MoreCharacter->MaxDamage[1]){
							//word MinR=OB->newMons->AdvChar->AttackRadius1[1];
							word MaxR=OB->Ref.General->MoreCharacter->AttackRadius2[1];
							word Dam=OB->Ref.General->MoreCharacter->MaxDamage[1];
...
bool DetAbsorbeAbility(word BuildNIndex, word UnitNIndex){
	...
	if(BuildNIndex<NMon && UnitNIndex<NMon){
		...
		return (Build->UnitAbsorber||Build->PeasantAbsorber&&Unit->Peasant)&&Build->NConcPt&&!(Unit->HighUnit&&Build->HighUnitCantEnter);
	}
	return false;
};
