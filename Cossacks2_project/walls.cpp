...
void WallCluster::ConnectToPoint(short x,short y,bool Vis){
    if(!NCells){
        ...
        do{
            if(NCells<2){
                //first sector
                if(!NCells)AddPoint(LastX,LastY,Vis);
                dx=x-LastX;
                dy=y-LastY;
                if(dx>1)dx=1;
                if(dy>1)dy=1;
	...
	if(Opdone){
		for(int i=0;i<NSel;i++){
			if(CurCell<NCells){
				...
				if(MID!=0xFFFF){
					OneObject* OB=Group[MID];
					if(OB&&OB->NNUM==NI)CmdDone[MID]=0xFFFF;
				};
			};
		};
	};
...
bool WallCell::StandOnLand(WallCluster* WC){
    NewMonster* NM=WC->NM;
    if(!(CheckPosition()&&ApplyCost(WC->NI,WC->NIndex,-1,-1))){
        Visible=false;
        return false;
    };
...
bool OneObject::BuildWall(int xx,int yy,byte Prio,byte OrdType,bool TempBlock){
	...
	if(FindPlaceForWallBuilder(NNUM,&xx,&yy,x>>2,y>>2)){
		...
		if(TempBlock)Or1->OrderType=123;
		else Or1->OrderType=122;
		Or1->OrderTime=0;
		Or1->DoLink=&BuildWallLink;
		Or1->info.MoveToXY.x=xx;
		Or1->info.MoveToXY.y=yy;
		if(!OrdType)if(TempBlock)BSetBar((xx<<2)+1,(yy<<2)+1,2);
...
void PlayOpenGate(Gate* Gat,WallCharacter* WCR){
	if(WCR->OpenGateSoundID==-1)return;
	if(div(WCR->OpenKeyFrame,HISPEED+1).quot==div(Gat->State,HISPEED+1).quot){
		int xx=(int(Gat->x)<<6)+32;
		int yy=(int(Gat->y)<<6)+32;
		AddEffect(xx,(yy>>1)-GetHeight(xx,yy),WCR->OpenGateSoundID);
	};
};
void PlayCloseGate(Gate* Gat,WallCharacter* WCR){
	if(WCR->CloseGateSoundID==-1)return;
	if(div(WCR->CloseKeyFrame,HISPEED+1).quot==div(Gat->State,HISPEED+1).quot){
		int xx=(int(Gat->x)<<6)+32;
		int yy=(int(Gat->y)<<6)+32;
		AddEffect(xx,(yy>>1)-GetHeight(xx,yy),WCR->CloseGateSoundID);
	};
};
void ControlGates(){
	ControlSquish();
	for(int i=0;i<NGates;i++){
		...
		if(Gat->NI!=0xFF){
			if(Gat->Locked){
				...
				//if(CheckNTPresence((int(Gat->x)<<6)+32,(int(Gat->y)<<6)+32,120,Gat->NMask)){
		...
		if((!gat->Locked)&&(gat->NI==NI||((gat->State>NGOpen-3)&&gat->NI!=0xFF))){
			...
			if(LI>=0&&LI<MaxLI){
				...
				if(WC){
					...
					case 1:
						/*
						MFIELDS->BClrSQ(x+2,y-1,3,3);
						MFIELDS->BClrSQ(x-1,y+2,3,3);
						MFIELDS->BClrPt(x+3,y-2);
						MFIELDS->BClrPt(x-2,y+3);
						MFIELDS->BClrPt(x+5,y);
						MFIELDS->BClrPt(x,y+5);
						MFIELDS->BClrPt(x+1,y);
						MFIELDS->BClrPt(x,y+1);
						MFIELDS->BClrPt(x+1,y+1);
						MFIELDS->BClrPt(x+2,y+2);
						MFIELDS->BClrPt(x+3,y+2);
						MFIELDS->BClrPt(x+2,y+3);
						*/
					...
					case 3:
						/*
						MFIELDS->BClrSQ(x-1,y-1,3,3);
						MFIELDS->BClrSQ(x+2,y+2,3,3);
						MFIELDS->BClrPt(x,y-2);
						MFIELDS->BClrPt(x-2,y);
						MFIELDS->BClrPt(x+2,y);
						MFIELDS->BClrPt(x,y+2);
						MFIELDS->BClrPt(x+2,y+1);
						MFIELDS->BClrPt(x+1,y+2);
						MFIELDS->BClrPt(x+3,y+1);
						MFIELDS->BClrPt(x+1,y+3);
						MFIELDS->BClrPt(x+5,y+3);
						MFIELDS->BClrPt(x+3,y+5);
						*/
						MFIELDS->BClrSQ(x,y-1,2,4);
						MFIELDS->BClrSQ(x+2,y+1,2,4);
						MFIELDS->BClrPt(x-1,y);
		...
		if(gat->NI==NI&&((gat->State<=NGOpen-3)&&gat->NI!=0xFF)){
			...
			if(LI>=0&&LI<MaxLI){
				...
				if(WC){
					...
					case 1:
						/*
						MFIELDS->BSetSQ(x+2,y-1,3,3);
						MFIELDS->BSetSQ(x-1,y+2,3,3);
						MFIELDS->BSetPt(x+3,y-2);
						MFIELDS->BSetPt(x-2,y+3);
						MFIELDS->BSetPt(x+5,y);
						MFIELDS->BSetPt(x,y+5);
						MFIELDS->BSetPt(x+1,y);
						MFIELDS->BSetPt(x,y+1);
						MFIELDS->BSetPt(x+1,y+1);
						MFIELDS->BSetPt(x+2,y+2);
						MFIELDS->BSetPt(x+3,y+2);
						MFIELDS->BSetPt(x+2,y+3);
						*/
					...
					case 3:
						/*
						MFIELDS->BSetSQ(x-1,y-1,3,3);
						MFIELDS->BSetSQ(x+2,y+2,3,3);
						MFIELDS->BSetPt(x,y-2);
						MFIELDS->BSetPt(x-2,y);
						MFIELDS->BSetPt(x+2,y);
						MFIELDS->BSetPt(x,y+2);
						MFIELDS->BSetPt(x+2,y+1);
						MFIELDS->BSetPt(x+1,y+2);
						MFIELDS->BSetPt(x+3,y+1);
						MFIELDS->BSetPt(x+1,y+3);
						MFIELDS->BSetPt(x+5,y+3);
						MFIELDS->BSetPt(x+3,y+5);
						*/
...
void SquishUnitsInCell(int cell,byte NMask,int x,int y,int R){
    cell+=VAL_MAXCX+1;
	if(cell<0||cell>=VAL_MAXCIOFS)return;
    int NMon=MCount[cell];
	if(!NMon)return;
	int ofs1=cell<<SHFCELL;
	word MID;
	for(int i=0;i<NMon;i++){
		...
		if(MID!=0xFFFF){
			OneObject* OB=Group[MID];
			if(OB&&(!OB->Sdoxlo)/*&&(!(OB->NMask&NMask))*/&&Norma(x-OB->RealX,y-OB->RealY)<R){
				if(MFIELDS->CheckBar(OB->x,OB->y,OB->Lx,OB->Lx))
					if(!(OB->Wall||OB->BlockInfo))OB->Die();
			};
		};
	};
};
void SquishUnits(int x,int y,int R,byte NMask){
    ...
    for(int nx=0;nx<rx2;nx++){
        for(int ny=0;ny<rx2;ny++){
            if(stcell>=0&&stcell<VAL_MAXCIOFS){
                if(bpt[0]){
                    SquishUnitsInCell(stcell,NMask,x,y,R);
                };
            };
            stcell++;
            bpt++;
...
extern int tmtmt;
void ControlSquish(){
	if(div(tmtmt,27).rem)return;
	for(int i=0;i<NGates;i++){
		Gate* Gat=Gates+i;
		if(Gat->NI!=0xFF){
			SquishUnits((int(Gat->x)<<6)+32,(int(Gat->y)<<6)+32,120,Gat->NMask);
		};
	};
};
