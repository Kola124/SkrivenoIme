...
void CBattle::MakeDialog(bool frame){
	...
	DString	TXT;	TXT.ReadFromFile("Battle\\TextLine.cml"); if(!TXT.str)return;
	DString	CTXT;	CTXT.ReadFromFile("Battle\\CTextLine.cml"); if(!CTXT.str)return;
	...
	DString UTP;	UTP.ReadFromFile("Battle\\UnitType.cml"); if(!UTP.str)return;
	DString UTA;	UTA.ReadFromFile("Battle\\untypean.cml"); if(!UTA.str)return;
	DString UTS;	UTS.ReadFromFile("Battle\\SelPan.cml"); if(!UTS.str)return;	
	DString UPS;	UPS.ReadFromFile("Battle\\Upgrade.cml"); if(!UPS.str)return;
	DString CHK;	CHK.ReadFromFile("Battle\\Check.cml"); if(!CHK.str)return;	
		...
		if(t==PInfo->SelUT){
			...
			for(;ctg!=ctgEnd;ctg++) if(ctg->Linked.size()+ctg->UnLinked.size()){				
				...
				while(lin!=linEnd){
					...
					if(u0!=0xFFFF){
						AddUpgToCml(CML, UPS.str, u0, 0, x+n*w, y);
						if(++n>=nx){
							y+=h;
							n=0;
						}
					...
					if(u1!=0xFFFF){
						AddUpgToCml(CML, UPS.str, u1, 0, x+n*w, y);
						if(++n>=nx){
							y+=h;
							n=0;
						}
				...
				while(unl!=unlEnd){
					...
					AddUpgToCml(CML, UPS.str, UIndex, 0, x+n*w, y);
...
void UnitType::GetCharacter(byte NI){	
	...
	if(NAttTypes){
		...
		CurShield=MC.Shield;
		CurR2.reserve(NAttTypes); for(int i=0;i<NAttTypes;i++) CurR2[i]=MC.AttackRadius2[i];
		CurAPause.reserve(NAttTypes); for(int i=0;i<NAttTypes;i++){
