...
void Draw_PLIST(int x,int y,int Lx,int Ly,int Index,byte Active,int param){
	...
	if(Index<CSYS.CCH[CSYS.CurChannel].NPlayers){
		...
		RLCFont* FONT=Active==1?&WhiteFont:&YellowFont;
		RLCFont* FONT1=NULL;
		char cc3[256];
...
void EnterPersonalMessage(char* Nick,bool Active){
	...
	GP_TextButton* OK=DSS.addGP_TextButton(NULL,X0+23,Y0+20,GetTextByID("@OK"),BTNS.GPID,82,&BlackFont,&BlackFont);
	...
	DSS.addGPPicture(NULL,X0,Y0,BTNS.GPID,81);
	//GPS.ShowGP(X0,Y0,BARS.GPID,17,0);
	GP_TextButton* CANCEL=DSS.addGP_TextButton(NULL,X0+23,Y0+20,GetTextByID("@CANCEL"),BTNS.GPID,82,&BlackFont,&BlackFont);
	...
	InputBox* IB=DSS.addInputBox(NULL,RealLx/2-143-10-DL,RealLy/2-4,MESSAGE,120,290+DL*2,20,&BlackFont,&BlackFont);
	IB->Active=1;
	char cc2[128];
	sprintf(cc2,GetTextByID("ENTERMESS"),nick);
	DSS.addTextButton(NULL,RealLx/2,RealLy/2-39,cc2,&BlackFont,&BlackFont,&BlackFont,1);
