...
void ChatSystem::AddPlayer(char* Nick,int c){
	...
	char* Inf=GetCUserInfo(Nick);
	if(Inf){
		strcpy(CCH[c].Players[CCH[c].NPlayers].Info,Inf);
...
void ChatSystem::AddAbsentPlayer(char* Nick){
	int i;
	for(int i=0;i<NAbsPlayers;i++)if(!strcmp(Nick,AbsPlayers[i].Nick))return;
	...
	char* Inf=GetCUserInfo(Nick);
	if(Inf){
		strcpy(AbsPlayers[NAbsPlayers].Info,Inf);
		AbsPlayers[NAbsPlayers].InfoReqSent=1;
	};
	NAbsPlayers++;
};
...
void GetUserInfoCallback(CHAT chat, CHATBool success, const char * nick, const char * user, const char * name, const char * address, int numChannels, const char ** channels, void * param)
{
	...
	if(CC){
		if(!success){
			...
			AddCUserInfo((char*)nick,(char*)user);
...
void ChatSystem::CheckMessage(char* message){
	...
	if(cc){
		...
		do{
			...
			PRESENT=0;
			for(int q=0;q<NCHNL;q++)for(int i=0;i<CCH[q].NPlayers;i++)
...
void ChatSystem::Process(){
	...
	int T0=GetTickCount();
	if(!PREVSEND)PREVSEND=T0-6000;
	if((T0-PREVSEND<400&&NSS)||(T0-PREVSEND<40&&!NSS))return;
	PREVSEND=T0;
	int NS=0;
	NSS=0;
	for(int j=0;j<NCHNL;j++){
		int NPL=CCH[j].NPlayers;
		if(NPL){
			int P=(GetTickCount()+rand())%NPL;
			bool V2=CCH[j].Players[P].Info[0]==0;
			if(V2){
				chatGetUserInfo(chat,CCH[j].Players[P].Nick,&GetUserInfoCallback,this,CHATFalse);
				CCH[j].Players[P].InfoReqSent=1;
				NS++;
				NSS++;
				return;
			};
...
bool ChatSystem::ConnectToChat(char* Nick,char* Info,char* Mail,char* Chat,char* C1,char* C2){
	...
	Connected=0;
	Setup();
	strcpy(chatChannel[0],C1);
	strcpy(chatChannel[1],C2);
