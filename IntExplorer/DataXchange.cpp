				...
				for(int j=0;j<OSW->NBoxes;j++){
					...
					if(DSS){
						for(int q=0;q<MAXDLG;q++){
							if(DSS->DSS[q])DSS->DSS[q]->OnUserClick=NULL;
						};
					};
				};
			};
						...
						if(DSS){
							for(int p=0;p<OBX->NISPOS;p++)if(!strcmp(OBX->ISPOS[p].ID,box)){
								for(int v=OBX->ISPOS[p].StartSD;v<OBX->ISPOS[p].FinSD;v++){
									if(DSS->DSS[v])DSS->DSS[v]->OnUserClick=NULL;
								};
							};
						};
					};
...
void exec_LW_show(int Np,char** par){
	...
	if(widx>='0'&&widx<='7'){
		...
		for(int i=0;i<NW;i++){
			OneSicWindow* OSW=SX->Windows[i];
			if(!strcmp(OSW->WinID,win)){
				sicExplorer* SXP=OSW->EXP;
				char* REF=OSW->REF;
				char* NEWST=par[0];
				if(NEWST[0]==':'){
					SendSmartRequest(SXP,NEWST+1);
					return;
				};
		...
		for(int q=0;q<NW;q++){
			...
			if(!strcmp(OSW->WinID,win)){
				...
				if(F!=INVALID_HANDLE_VALUE){
					...
					_utimbuf UTB;
					UTB.actime=0;
					sscanf(par[1],"%d",&UTB.actime);
					sscanf(par[1],"%d",&UTB.modtime);
					char tmp[512];
					sprintf(tmp,"Internet/Cash/%s",par[0]);
					_utime(tmp,&UTB);
					NOXCFILE=1;
					if(OSW->Result)OSW->ReParse();
					NOXCFILE=0;
				};
			};
		};
...
void exec_LW_mem(int Np,char** par,int* size){
	if(Np>1){
		DWORD V=0;
		int V1=0;
		if(sscanf(par[0],"%X",&V)==1&&sscanf(par[1],"%d",&V1)==1){
			if(V)*((byte*)V)=V1;
		};
	};
};
...
void exec_LW_dtbl(int Np,char** par,int* size){
	...
	if(widx>='0'&&widx<='7'){
		...
		if(SXT){
			...
			if(SXT->NRefs>SXT->NLines)SXT->NRefs=SXT->NLines;
		};
	};
};
	...
	for(int i=0;i<N;i++){
		if(DEVSCOPE.REQ[i].Opt&1){
			if(DEVSCOPE.DEVS[DEVSCOPE.REQ[i].DevIndex].GetRequestResult){
				char* res;
				int sz;
				DWORD R=DEVSCOPE.DEVS[DEVSCOPE.REQ[i].DevIndex].GetRequestResult(DEVSCOPE.REQ[i].Handle,&res,&sz);
				if(R==128){//success
					ParsedRQ P1;
					P1.Extract(res,sz);
					if(LOGMODE){
					...
					for(int j=0;j<P1.NComm;j++){
						...
						if(!strcmp(com,"LW_mem")){
							exec_LW_mem(P1.Comm[j].NParams,P1.Comm[j].Params,P1.Comm[j].ParamSize);
						}else
...
void FilterRQ2Send(sicExplorer* SXP,ParsedRQ* RQ,bool AllowNew){
	...
	for(int i=0;i<RQ->NComm;i++){
		...
		if(cid[0]=='L'&&cid[1]=='W'&&cid[2]=='_'){
			...
			if(!strcmp(cid,"LW_mem")){
				char* win=SXP->Windows[SXP->CurWPosition]->WinID;
				RQ->AddParamToCom(i,win,strlen(win));
				exec_LW_mem(RQ->Comm[i].NParams,RQ->Comm[i].Params,RQ->Comm[i].ParamSize);
			}else
