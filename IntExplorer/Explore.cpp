...
RLCFont* sicExplorer::GetFontByName(char* fnt){
	if(CurWPosition<NWindows){
		...
		for(int j=0;j<OSW->NAddFonts;j++)if(!strcmp(fnt,OSW->ADFonts[j].FID))return &OSW->ADFonts[j].FONT;
	};
		...
		if(c=='#'){
			...
			if(B1||B2||B3){
				...
				if(Result[pos]=='%'){
					...
					int Y1=EXP->y1;
					char* src=Result+pos;
					ReadCoordinates(&src,X0,Y0,X1,Y1,NULL);
					...
					if(B2){
						...
						if(GPIDX!=-1){
							...
							if(gpid!=-1){
								OBX->VS=OBX->DSS.addNewGP_VScrollBar(NULL,
...
OneSicWindow::~OneSicWindow(){
	Erase();
	if(Result)free(Result);//bug is there!!
};
OneBox::OneBox(){
	memset(this,0,sizeof *this);
};
...
char* OneSicWindow::CreateVar(char* Name,int Len){
	...
	VARS[NVARS].Value=znew(char,Len);
...
char* sicExplorer::CreateGVar(char* Name,int Len){
	...
	GVARS[NGVars].Value=znew(char,Len);
...
char* sicExplorer::CreateVar(char* Name,int Len){
	if(NWindows)return Windows[CurWPosition]->CreateVar(Name,Len);
	return NULL;
};
...
void sicExplorer::ChangeOutput(int x,int y,int x1,int y1){
	...
	for(int j=0;j<NWindows;j++){
		...
		for(int i=0;i<OW->NBoxes;i++){
			...
			OW->Boxes[i].ISPOS=NULL;
			OW->Boxes[i].NISPOS=0;
			OW->Boxes[i].MaxISPOS=0;
		};
		//for(int i=0;i<OW->NVARS;i++){
...
void sicExplorer::StepBack(){
	if(CurWPosition>0)CurWPosition--;
};
void sicExplorer::StepForward(){
	if(CurWPosition<NWindows-1)CurWPosition++;
};
void sicExplorer::Refresh(){
	if(CurWPosition<NWindows){
		if(Windows[CurWPosition]&&Windows[CurWPosition]->REF){
			...
			sicExplorer* SXP=Windows[CurWPosition]->EXP;
			Windows[CurWPosition]->Erase();
			Windows[CurWPosition]->EXP=SXP;
...
void InitSXP(){ 
	...
	InitIFNS();
	InitDevs();
	for(int i=0;i<8;i++)SXP[i].ID=i;
};
extern "C" __declspec(dllexport)
void RunSXP(int Index,char* home,int x,int y,int x1,int y1){
	if(Index>8)return;
	//SXP[Index].CloseAll();
	if(Index==0&&SXP->NWindows)return;
	SXP[Index].ID=Index;
...
extern "C" __declspec(dllexport)
void OpenRef(int Index,char* home){
	if(Index>8)return;
	SendGlobalRequest(SXP+Index,home,1);
	if(SXP[Index].CurWPosition<SXP[Index].NWindows){
		SXP[Index].Windows[SXP[Index].CurWPosition]->Process();
...
CEXPORT
void SetAccessKey(int Index,char* Accesskey){
...
extern "C" __declspec(dllexport)
char* SXP_GetVar(int Index,char* Name){
	sicExplorer* EXP=SXP+Index;
