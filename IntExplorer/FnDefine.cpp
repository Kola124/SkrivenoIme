	...
	if(NParam){
		...
		do{
			...
			if(wasactive!=curactive||GP_Idx!=-1||xcs0+cslx+reallx>Lx||eof||eoln){
				...
				}else TB=DSS->addTextButton(NULL,x0+xcs0,y0,curstr,PFONT,PFONT,PFONT,0);
				SDS[NSDS]=TB;
				NSDS++;
				//new string?
...
bool ADI_Edit(sicExplorer* SXP,DialogsSystem* DSS,int* x,int* y,int* x1,int* y1,
	...
	DSS->addInputBox(NULL,*x,*y,VAR,500,(*x1)-(*x),(*y1)-(*y),SXP->GetFontByName(SXP->FONT1),SXP->GetFontByName(SXP->FONT3));
	return true;
};
struct PingST{
	TextButton* TB;
...
bool ADI_APan(sicExplorer* SXP,DialogsSystem* DSS,int* x,int* y,int* x1,int* y1,
	...
	if(NParam>1){
		...
		RLCFont* AFONT=SXP->GetFontByName(SXP->FONT2);
		RLCFont* MFONT=SXP->GetFontByName(SXP->FONT3);
		DSS->addTextButton(NULL,(*x+*x1)/2,((*y+*y1-GetRLCHeight(PFONT->RLC,'W'))/2)-1,Param[1],AFONT,PFONT,MFONT,1);
	};
	if(NActive){
...
bool ADI_AddDBTBL(sicExplorer* SXP,DialogsSystem* DSS,int* x,int* y,int* x1,int* y1,
	...
	for(int i=0;i<ADI->NColumns;i++)if(ADI->COLM[i].Width){
		...
		if(ADI->Style&1){
			...
			CheckBox* CB;
			CB=DSS->addGP_CheckBox(NULL,xx+OSW->b_DxL,(*y)+OSW->b_DyL,ADI->COLM[i].Message,AFONT,PFONT,1,0,CurPalette?IBOR2:IBOR0,26,23,20);
			CB->Central=1;
		...
		if(ADI->Style&2){
			...
			Ty1-=OSW->CTBL_LowLY;
			if(ft>1&&ft<=4){
				ComboBox* CBB=DSS->addGP_ComboBoxDLX(NULL,xx+OSW->b_DxL,(*y1)-OSW->CTBL_LowLY+1,xx1-xx-OSW->b_DxR-OSW->b_DxL+dw/*+24-OSW->cmb_bdx*/,comidx,OSW->cmb_Idx,9,0,AFONT,PFONT,ADI->COLM[i].FiltParam);
